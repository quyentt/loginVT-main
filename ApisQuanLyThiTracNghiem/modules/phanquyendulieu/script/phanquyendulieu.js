function phanquyendulieu() { }
phanquyendulieu.prototype = { 
    strNguoiDung_Id: '', 
    strDonViId:'',
    dtNguoiDungDonVi: [],
    dtNguoiDungMucPheDuyet: [],
    ...
    init: function () {
        ...
        $("[id$=chkSelectAll_NguoiDungDonVi]").on("click", function () {
            edu.util.checkedAll_BgRow(this, { table_id: "tblMucPheDuyetNguoiDung" });
        });
        ...
        $(".btnThem_Muc_DonVi_PhanQuyen").click(function () {
            ...
            $("#btnYes").click(function (e) {
                $('#myModalAlert #alert_content').html('');
                for (var i = 0; i < me.dtNguoiDungMucPheDuyet.length; i++) {
                    console.log($('#checkPQMucDonVi' + me.dtNguoiDungMucPheDuyet[i].ID).is(':checked'));
                    var strCoQuyen = "0";
                    if ($('#checkPQMucDonVi' + me.dtNguoiDungMucPheDuyet[i].ID).is(':checked'))
                        strCoQuyen = "1";
                    me.Them_MucNguoiDungDonVi(me.dtNguoiDungMucPheDuyet[i].ID, strCoQuyen);
                }
           });
            setTimeout(function () {
                me.getList_NguoiDungMucPheDuyet(); 
                me.getList_NguoiDungDonVi();
            }, 2000);
        }); 
    },
    ...
    genTable_NguoiDung: function (data, iPager) {
        var me = main_doc.phanquyendulieu;
        edu.util.viewHTMLById("tblNguoiDung_Tong", iPager);
        var jsonForm = {
            strTable_Id: "tblNguoiDung_NDDV",
            aaData: data,
            bPaginate: {
                strFuntionName: "main_doc.phanquyendulieu.getList_NguoiDung()",
                iDataRow: iPager
            },
    ...
    genList_NguoiDungDonVi: function (data, iPager) {
        ...
        var jsonForm = {
            ...
            aaData: data,
            bPaginate: {
                strFuntionName: "main_doc.phanquyendulieu.getList_NguoiDungDonVi()",
                iDataRow: iPager,
                bInfo: false,
                bLeft: false
            },
            ...
            aoColumns: [
                ...
                {
                    "mDataProp": "TENNGUOIDUNGCAPPHANQUYEN"
                }, 
                {
                    "mRender": function (nRow, aData) {
    ...
    genList_DonViChuaPhanQuyen: function (data, iPager) {
        ...
        var jsonForm = {
            ...
            aaData: data,
            bPaginate: {
                strFuntionName: "main_doc.phanquyendulieu.getList_DonViChuaPhanQuyen()",
                iDataRow: iPager,
                bInfo: false,
                bLeft: false
            },
    ...
    getList_NguoiDungMucPheDuyet: function () {
        var me = this;
        //--Edit
        var obj_list = {
            'action': 'QLTTN_ThongTin/LayDS_NguoiDungMucPheDuyet',
            'versionAPI': 'v1.0',
            'strUserId': me.strNguoiDung_Id,
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) { 
                    me.dtNguoiDungMucPheDuyet = data.Data;
                    me.genList_NguoiDungMucPheDuyet(data.Data);
                }
                else {
                    edu.system.alert(obj_list.action + " (er): " + JSON.stringify(data.Message), "w");
                }
    ...
    genList_NguoiDungMucPheDuyet: function (data) {
        var me = this;
        
        var jsonForm = {
            strTable_Id: "tblMucPheDuyetNguoiDung",
            aaData: data,            
            sort: true,
            colPos: {
                center: [0, 1]
            ...
            aoColumns: [
                ...
                {
                    "mRender": function (nRow, aData) {
                        if (aData.DAPHANQUYEN > 0)
                            return '<input type="checkbox" checked id="checkPQMucDonVi' + aData.ID + '"/>';
                        else
                            return '<input type="checkbox" id="checkPQMucDonVi' + aData.ID + '"/>';
                    }
                }
            ]
        };
