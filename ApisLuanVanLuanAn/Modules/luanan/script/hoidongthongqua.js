...
--Date of updated:
----------------------------------------------*/
function HoiDongThongQua() { };
HoiDongThongQua.prototype = {
    strHoiDongThongQua_Id: '',
    dtHoiDongThongQua: [],
    dtHeDaoTao: [],
    dtKhoaDaoTao: [],
    arrNhanSu_Id: [],
    ...
    init: function () {
        ...
        $("#btnSave").click(function () {
            me.save_HoiDongThongQua();
        });
        $(".btnDelete").click(function () {
            if (edu.util.checkValue(me.strHoiDongThongQua_Id)) {
                edu.system.confirm("Bạn có chắc chắn xóa dữ liệu không?");
                $("#btnYes").click(function (e) {
                    me.delete_HoiDongThongQua();
                });
                return false;
            }
            else {
        ...
        $("#tblHoiDongThongQua").delegate(".btnEdit", "click", function () {
            var strId = this.id;
            if (edu.util.checkValue(strId)) {
                me.toggle_form_input();
                me.strHoiDongThongQua_Id = strId;
                me.getList_HoiDongThongQua_ThanhVien();
                //me.getList_HoiDongThongQua_HuongDan();
                edu.util.setOne_BgRow(strId, "tblHoiDongThongQua");
                me.viewForm_HoiDongThongQua(edu.util.objGetOneDataInData(strId, me.dtHoiDongThongQua, "ID"));
                edu.system.hiddenElement('{"readonlyselect2": "#dropHeDaoTao,#dropKhoaDaoTao,#dropChuongTrinh,#dropLop,#dropHocVien,#dropPhanLoai","readonly": "#txtSoQuyetDinh,#txtNgayQuyetDinh,#txtTenDeCuongTiengViet,#txtTenDeCuongTiengAnh,#txtNgayDangKy,#txtMoTa"}');
            }
            else {
                edu.system.alert(edu.constant.getting("NOTIFY", "SELECT_F"));
            }
        ...
        $("#tblInputDanhSachNhanSu").delegate('.btnDeletePoiter', 'click', function () {
            ...
            if (!edu.util.checkValue(strcheck)) {
                ...
                $("#btnYes").click(function (e) {
                    me.delete_HoiDongThongQua_ThanhVien(strNhanSu_Id);
                });
            }
        });
        /*------------------------------------------
        ...
        $("#btnThemHeKhoa").click(function () {
            ...
            me.genHTML_HoiDongThongQua_HeKhoa(id, "");
        });
        $("#zone_input_HoiDongThongQua").delegate(".deleteRowButton", "click", function () {
            var strRowId = this.id;
            $("#tbl_HeKhoa tr[id='" + strRowId + "']").remove();
        });
        $("#zone_input_HoiDongThongQua").delegate(".deleteHeKhoa", "click", function () {
            var strId = this.id;
            edu.system.confirm(edu.constant.getting("NOTIFY", "CF_DELETE"));
            $("#btnYes").click(function (e) {
                me.delete_HoiDongThongQua_HeKhoa(strId);
            });
        });
    ...
    page_load: function () {
        ...
        edu.system.loadToCombo_DanhMucDuLieu("LVLA.VTHDONG", "", "", function (data) {
            main_doc.HoiDongThongQua.dtVaiTro = data;
        });
        edu.system.loadToCombo_DanhMucDuLieu("LVLA.PLCD", "dropPhanLoai");
        edu.system.loadToCombo_DanhMucDuLieu("LVLA.DGHD", "", "", function (data) {
            main_doc.HoiDongThongQua.dtDanhGia = data;
        });
    },

    toggle_notify: function () {
        edu.util.toggle_overide("zone-bus", "zone_notify_HoiDongThongQua");
    },
    toggle_form_input: function () {
        edu.util.toggle_overide("zone-bus", "zone_input_HoiDongThongQua");
    },
    ...
    getList_HoiDongThongQua: function () {
        var me = this;

        //--Edit
        var obj_list = {
            'action': 'LVLA_DangKy_ChuyenDe/LayDanhSach',
            'strTuKhoa': edu.util.getValById('txtSearch_TuKhoa'),
            'strQLSV_NguoiHoc_Id': edu.util.getValById('dropAAAA'),
            'strNguoiDangKy_Id': edu.util.getValById('dropAAAA'),
            'strLVLA_KeHoach_ChuyenDe_Id': edu.util.getValById('dropSearch_KeHoach'),
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    me.dtHoiDongThongQua = dtResult;
                    me.genTable_HoiDongThongQua(dtResult, iPager);
                }
                else {
    ...
    save_HoiDongThongQua: function () {
        var me = this;
        var strId = me.strHoiDongThongQua_Id;
        $("#tblInputDanhSachNhanSu tbody tr").each(function () {
            var strNhanSu_Id = this.id.replace(/rm_row/g, '');
            me.save_HoiDongThongQua_ThanhVien(strNhanSu_Id, strId, $(this).attr("name"));
        });
        me.getList_HoiDongThongQua();
    },
    delete_HoiDongThongQua: function () {
        var me = this;
        //--Edit
        var obj_delete = {
            'action': 'LVLA_DeCuong_DangKy/Xoa',

            'strIds': me.strHoiDongThongQua_Id,
            'strNguoiThucHien_Id': edu.system.userId
        };
        //default
    ...
    --ULR:  Modules
    -------------------------------------------*/
    genTable_HoiDongThongQua: function (data, iPager) {
        var me = this;
        $("#lblHoiDongThongQua_Tong").html(iPager);
        var jsonForm = {
            strTable_Id: "tblHoiDongThongQua",
            aaData: data,
            bPaginate: {
                strFuntionName: "main_doc.HoiDongThongQua.getList_HoiDongThongQua()",
                iDataRow: iPager
            },
            sort: true,
            colPos: {
    ...
    viewForm_HoiDongThongQua: function (data) {
        var me = main_doc.HoiDongThongQua;
        //view data --Edit
        ...
        edu.util.viewValById("dropSearch_KeHoach", data.LVLA_KEHOACH_CHUYENDE_ID);
    ...
    --ULR:  Modules
    -------------------------------------------*/
    getList_HoiDongThongQua_HuongDan: function () {
        var me = this;

        //--Edit
        var obj_list = {
            'action': 'LVLA_BaoVe_ChuyenDe_HoiDong_ThanhVien/LayDanhSach',
            'strTuKhoa': edu.util.getValById('txtAAAA'),
            'strNguoiDung_Id': edu.util.getValById('dropAAAA'),
            'strLVLA_DangKy_ChuyenDe_Id': me.strHoiDongThongQua_Id,
            'strNguoiThucHien_Id': edu.system.userId,
            'pageIndex': 1,
            'pageSize': 100000,
        };
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    me.genTable_HoiDongThongQua_HuongDan(dtResult);
                }
                else {
                    edu.system.alert(obj_list.action + ": " + data.Message, "w");
                }
    ...
    genTable_HoiDongThongQua_HuongDan: function (data) {
        var me = this;
        //3. create html
    ...
    --ULR:  Modules
    -------------------------------------------*/
    save_HoiDongThongQua_ThanhVien: function (strNhanSu_Id, strHoiDongThongQua_Id, strId) {
        var me = this;
        ...
        var obj_save = {
            'action': 'LVLA_BaoVe_ChuyenDe_HoiDong_ThanhVien/ThemMoi',


            'strId': strId,
            'strChucNang_Id': edu.system.strChucNang_Id,
            'strLVLA_DangKy_ChuyenDe_Id': strHoiDongThongQua_Id,
        ...
        if (edu.util.checkValue(obj_save.strId)) {
            obj_save.action = 'LVLA_BaoVe_ChuyenDe_HoiDong_ThanhVien/CapNhat';
        }
        //default
    ...
    getList_HoiDongThongQua_ThanhVien: function () {
        var me = this;
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    me.genTable_HoiDongThongQua_ThanhVien(dtResult);
                }
                else {
                    edu.system.alert(obj_list.action + ": " + data.Message, "w");
                }
    ...
    delete_HoiDongThongQua_ThanhVien: function (strNhanSu_Id) {
        var me = this;
        //--Edit
        var obj = {};
        var obj_delete = {
            'action': 'LVLA_BaoVe_ChuyenDe_HoiDong_ThanhVien/Xoa',
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    obj = {
                        content: "TS_HoiDongThongQua_NhanSu/Xoa: " + data.Message,
                        code: ""
                    };
                    edu.system.afterComfirm(obj);
                }
            ...
            error: function (er) {
                var obj = {
                    content: "TS_HoiDongThongQua_NhanSu/Xoa (er): " + JSON.stringify(er),
                    code: "w"
                };
                edu.system.afterComfirm(obj);
    ...
    --ULR:  Modules
    -------------------------------------------*/
    genTable_HoiDongThongQua_ThanhVien: function (data) {
        var me = this;
        //3. create html
        me.arrNhanSu_Id = [];
        $("#tblInputDanhSachNhanSu tbody").html("");
        for (var i = 0; i < data.length; i++) {
            var strNhanSu_Id = data[i].THANHVIEN_ID;
            var html = "";
            html += "<tr id='rm_row" + strNhanSu_Id + "' name='" + data[i].ID + "'>";
            html += "<td class='td-center'>" + (i + 1) + "</td>";
            ...
            html += '<td><input value="' + edu.util.returnEmpty(data[i].DIEM) + '" type="text" id="txtDiem' + strNhanSu_Id + '" class="form-control" /></td>';
            html += "<td class='td-left'><select id='danhgia_" + data[i].THANHVIEN_ID + "'></select></td>";
            html += '<td><input value="' + edu.util.returnEmpty(data[i].NHANXET) + '" type="text" id="txtNhanXet' + strNhanSu_Id + '" class="form-control" /></td>';
            ...
            me.genCombo_VaiTro("vaitro_" + strNhanSu_Id, data[i].VAITRO_ID);
            me.genCombo_DanhGia("danhgia_" + strNhanSu_Id, data[i].DANHGIA_ID);
