...
KeHoach.prototype = {
    ...
    save_KeHoach: function () {
        ...
        //--Edit
        var obj_save = {
            'action': 'LVLA_KeHoach_DeCuong/ThemMoi',
        ...
        if (obj_save.strId !== "") {
            obj_save.action = 'LVLA_KeHoach_DeCuong/CapNhat';
        }
        //default
    ...
    delete_KeHoach: function () {
        ...
        //--Edit
        var obj_delete = {
            'action': 'LVLA_KeHoach_DeCuong/Xoa',
    ...
    genTable_KeHoach: function (data, iPager) {
        ...
        //me.reportAllTable(jsonForm.strTable_Id)
        /*III. Callback*/
    },
    ...
    save_KeHoach_ThanhVien: function (strNhanSu_Id, strKeHoach_Id) {
        ...
        var obj_save = {
            'action': 'LVLA_KeHoach_DeCuong_NhanSu/ThemMoi',
    ...
    getList_KeHoach_ThanhVien: function () {
        ...
        //--Edit
        var obj_list = {
            'action': 'LVLA_KeHoach_DeCuong_NhanSu/LayDanhSach',
    ...
    delete_KeHoach_ThanhVien: function (strNhanSu_Id) {
        ...
        var obj = {};
        var obj_delete = {
            'action': 'LVLA_KeHoach_DeCuong_NhanSu/Xoa',
    ...
    reportAllTable: function (strTable_Id) {
        ...
        var head = $("#" + strTable_Id + " thead")[0].rows;
        var i = 0;
        for (i = 0; i < head.length; i++) {
            for (var j = 0; j < head[i].cells.length - 1; j++) {
                var cell = head[i].cells[j];
                objReportTable.lTable.push({
                    iRow: i,
                    iCol: j + cell.colSpan - 1,
                    iRowSpan: cell.rowSpan,
                    iColSpan: cell.colSpan,
                    strData_Cell: cell.innerHTML,
                    strData_Align: "HEAD"
                });
            }
        }
        var body = $("#" + strTable_Id + " tbody")[0].rows;
        for (var k = 0; k < body.length; k++) {
            for (var j = 0; j < body[k].cells.length - 1; j++) {
                objReportTable.lTable.push({
                    iRow: k + i,
                    iCol: j,
                    iRowSpan: 1,
                    iColSpan: 1,
                    strData_Cell: body[k].cells[j].innerHTML,
                    strData_Align: "BODY"
                });
            }
        }
        console.log(objReportTable);
        $.ajax
            ({
                url: "http://localhost:6368/Modules/Common/BaoCao.aspx",
                method: 'POST',
                contentType: false,
                processData: false,
                data: objReportTable,
