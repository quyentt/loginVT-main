...
PhanQuyenDiemLQL.prototype = {
    ...
    init: function () {
        ...
        -------------------------------------------*/
        edu.system.loadToCombo_DanhMucDuLieu("CHUNG.HANHDONG", "", "", data => me.genData_CotDuLieu(data));
        ...
        $("#btnPhanQuyenDiemLQL").click(function () {
            ...
            var x = $("#tblViewCauTrucPhanQuyenDiemLQL .chkApDungCaLop");
            for (var i = 0; i < x.length; i++) {
                if (($(x[i]).is(':checked') && $(x[i]).attr("name") == undefined) || (!$(x[i]).is(':checked') && $(x[i]).attr("name") != undefined)) {
                    let tId = x[i].id.replace(/chkApDung_/g, '');
                    console.log(tId)
                    var xP = $("#tblViewCauTrucPhanQuyenDiemLQL .check" + tId);
                    for (var y = 0; y < xP.length; y++) {
                        if ($(xP[y]).is(':checked')) {
                            if ($(xP[y]).attr("name") != undefined) {
                                arrThem.push(xP[y]);
                            }
                        }
                    }
            ...
            if ((arrThem.length + arrXoa.length) > 0) {
                ...
                $("#btnYes").click(function (e) {
                    ...
                    //edu.system.genHTML_Progress("divprogessPhanQuyenDiemLQL", arrQuyen_Id.length * arrHanhDong_Id.length);
                    for (var i = 0; i < arrThem.length; i++) {
                        var strId = arrThem[i].id;
                        var arrId = strId.split('_');
                        var aData = me.dtHangDuLieu.find(e => e.ID == arrId[1]);
                        me.save_PhanQuyenDiemLQL(aData.NGUOIDUNG_ID, aData.PHAMVIAPDUNG_ID, arrId[2], aData.HEDAOTAO_ID, $('#chkApDung_' + aData.ID).is(":checked") ? 1 : 0);
                    }
                    for (var i = 0; i < arrXoa.length; i++) {
                        var strId = arrXoa[i].id;
                        var arrId = strId.split('_');
                        var aData = me.dtHangDuLieu.find(e => e.ID == arrId[1]);
                        me.delete_PhanQuyenDiemLQL(aData.NGUOIDUNG_ID, aData.PHAMVIAPDUNG_ID, arrId[2], aData.HEDAOTAO_ID);
                    }
                });
            }
            else {
        ...
        $("#btnSave_KeHoachXuLy").click(function (e) {
            var arrChecked_Id = edu.util.getArrCheckedIds("tblPhamViPhanQuyen", "checkX");
            var arrQuyen_Id = edu.util.getArrCheckedIds("tblQuyen", "checkX");
            let tcheck = $('#dApDungCaLop').is(":checked") ? 1 : 0
            let tHe = edu.system.getValById('dropSearch_HeDaoTao2')

            $("#tblInputDanhSachNhanSu tbody tr").each(function () {
                var strNhanSu_Id = $(this).attr("name");
                arrChecked_Id.forEach(e => arrQuyen_Id.forEach(ele => me.save_PhanQuyenDiemLQL(strNhanSu_Id, e, ele, tHe, tcheck)))
            });
        });
    ...
    cbGenCombo_HeDaoTao: function (data) {
        ...
        var obj = {
            ...
            renderPlace: ["dropSearch_HeDaoTao", "dropSearch_HeDaoTao2"],
            type: "",
    ...
    getList_CotDuLieu: function () {
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    me.dtCot = data.Data;
                    me.genData_CotDuLieu(data.Data, data.Pager);
                    //me.genComBo_HS();
                }
                else {
                    edu.system.alert(obj_list.action + " : " + data.Message, "w");
    ...
    getList_HangDuLieu: function (strDanhSach_Id) {
        var me = this;
        //--Edit
        var obj_save = {
            'action': 'CMS_PhanQuyenDuLieu_MH/DSA4BRIVKSgeESkgIikeEDQ4JC8eBSgkLAPP',
            'func': 'pkg_chung_phanquyendulieu.LayDSThi_Phach_Quyen_Diem',
            'iM': edu.system.iM,
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    me["dtHangDuLieu"] = dtReRult;
                    me.genTable_HangDuLieu(dtReRult);
                }
                else {
    ...
    genTable_HangDuLieu: function (data, iPager) {
        var me = this;
        var strTable_Id = "tblViewCauTrucPhanQuyenDiemLQL";
        ...
        var jsonForm = {
            ...
            aoColumns: [
                ...
                {
                    "mRender": function (nRow, aData) {
                        let tcheck = aData.APDUNGQUYENCHOLOPHOCPHAN ? 'checked="checked" name="1"' : '';
                        return '<input type="checkbox" id="chkApDung_' + aData.ID + '" class="chkApDungCaLop checkApDungCaLop" ' + tcheck +' />';
                    }
                }
            ]
        };
            ...
            jsonForm.aoColumns.push(
                {
                    "mRender": function (nRow, aData) {
                        ...
                        return '<input type="checkbox" id="chk_' + aData.ID + '_' + main_doc.PhanQuyenDiemLQL.dtDoiTuong_View[iThuTu].ID + '" class="checkPhanQuyenDiemLQL check' + main_doc.PhanQuyenDiemLQL.dtDoiTuong_View[iThuTu].ID + ' check' + aData.ID + '" />';
                    }
                }
            );
    ...
    getList_KetQua_BangDuLieu: function (aData) {
        ...
        //--Edit
        var obj_save = {
            'action': 'CMS_PhanQuyenDuLieu_MH/DSA4BRIQNDgkLw8mNC4oBTQvJhEpICwXKAPP',
            'func': 'pkg_chung_phanquyendulieu.LayDSQuyenNguoiDungPhamVi',
            'iM': edu.system.iM,
            'strTuKhoa': edu.util.getValById('txtAAAA'),
            'strNguoiDung_Id': aData.NGUOIDUNG_ID,
            'strPhamViApDung_Id': aData.PHAMVIAPDUNG_ID,
            'strDaoTao_HeDaoTao_Id': aData.HEDAOTAO_ID,
            'strNguoiThucHien_Id': edu.system.userId,
        };
        //
            ...
            success: function (data) {
                if (data.Success) {
                    ...
                    for (var i = 0; i < dtReRult.length; i++) {
                        var check = $("#chk_" + aData.ID + "_" + dtReRult[i].HANHDONG_ID);
    ...
    --Discription: lop hoc
    ----------------------------------------------*/
    save_PhanQuyenDiemLQL: function (strNguoiDung_Id, strPhamViApDung_Id, strHanhDong_Id, strDaoTao_HeDaoTao_Id) {
        var me = this;
        //--Edit
        var obj_save = {
            'action': 'CMS_PhanQuyenDuLieu_MH/FSkkLB4VKSgeESkgIikeEDQ4JC8eBSgkLAPP',
            'func': 'pkg_chung_phanquyendulieu.Them_Thi_Phach_Quyen_Diem',
            'iM': edu.system.iM,
            ...
            'dApDungQuyenChoCaLHP': dApDungQuyenChoCaLHP,
            'strNguoiThucHien_Id': edu.system.userId,
        };
        //default
    ...
    delete_PhanQuyenDiemLQL: function (strNguoiDung_Id, strPhamViApDung_Id, strHanhDong_Id, strDaoTao_HeDaoTao_Id) {
        var me = this;
        //--Edit
        var obj_delete = {
            'action': 'CMS_PhanQuyenDuLieu_MH/GS4gHhUpKB4RKSAiKR4QNDgkLx4FKCQs',
            'func': 'pkg_chung_phanquyendulieu.Xoa_Thi_Phach_Quyen_Diem',
            'iM': edu.system.iM,
            'strNguoiDung_Id': strNguoiDung_Id,
            'strPhamViApDung_Id': strPhamViApDung_Id,
            'strHanhDong_Id': strHanhDong_Id,
    ...
    genComBo_CCTC: function (data) {
        var obj = {
            ...
            renderPlace: ["dropSearch_DonVi", "dropSearch_DonVi2"],
            type: "",
            title: "Chọn đơn vị"
        };
        edu.system.loadToCombo_data(obj);
        var jsonForm = {
            strTable_Id: "tblPhamViPhanQuyen",
    ...
    getList_HS: function (strDefaultVal) {
        ...
        var obj = {
            ...
            pageIndex: 1,
            pageSize: 1000000,
            strCoCauToChuc_Id: edu.util.getValById("dropSearch_DonVi"),
            strTinhTrangNhanSu_Id: "",
            strNguoiThucHien_Id: "",
            'dLaCanBoNgoaiTruong': 0
        };
        edu.system.getList_NhanSu(obj, "", "", me.genComBo_HS);
        ...
        var obj = {
            ...
            renderInfor: {
                ...
                Render: function (nrow, aData) {
                    return "<option id='" + edu.system.getRootPathImg(aData.HINHDAIDIEN) + "'class='table-img' value='" + aData.ID + "'>" + aData.MASO + " - " + edu.util.returnEmpty(aData.HOTEN) + "</option>";
                }
            },
