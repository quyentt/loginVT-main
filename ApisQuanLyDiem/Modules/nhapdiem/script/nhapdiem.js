...
NhapDiem.prototype = {
    ...
    init: function () {
        ...
        $(".btnSaveDSDiem").click(function () {
            ...
            $("#btnYes").click(function (e) {
                ...
                var arrElement = $("#" + strTable_Id).find("tbody").find("tr").find("td").find(".inputNhapDiem");
                var arrSave = [];
       ...
       -------------------------------------------*/
        ...
        $(".btnSaveXacNhanGiaHan").click(function () {
            ...
            $("#modal_XacNhan").modal("show");
            $(".loaiXacNhan").html("Xác nhận gia hạn");
            me.strLoaiXacNhan = "XACNHAN_GIAHAN";
            me.getList_BtnXacNhanSanPham("#zoneBtnXacNhan", me.strLoaiXacNhan);
            me.arrXacNhan_Id = [];
            arrChecked_Id.forEach(e => {
                let aData = me.dtNguoiHoc.find(ele => ele.ID == e);
                me.arrXacNhan_Id.push(aData.DIEM_DANHSACHHOC_ID + aData.QLSV_NGUOIHOC_ID)

            })
            me.getList_XacNhan(me.arrXacNhan_Id[0], "tblModal_XacNhan", null, me.strLoaiXacNhan);
        });
        $(".btnLayLaiDiem").click(function () {
            me.save_LayLaiDiem();
        });

        $(".btnXacNhanDiemDanh").click(function () {
            $("#modal_XacNhan").modal("show");
            $(".loaiXacNhan").html("Xác nhận");
        ...
        $("#zoneBtnXacNhan").delegate('.btnxacnhan', 'click', function () {
            ...
            if (me.strLoaiXacNhan == "XACNHAN_GIAHAN") {
                me.arrXacNhan_Id.forEach(e => me.save_XacNhanSanPham(e, strTinhTrang, strMoTa, me.strLoaiXacNhan));
            } else {
        ...
        edu.system.getList_MauImport("zonebtnBaoCao_Diem", function (addKeyValue) {
            var obj_list = {
                ...
                'strDangKy_KeHoachDangKy_Id': edu.util.getValById('dropSearch_KeHoachDKH'),
            ...
            for (var i = 0; i < arrChecked_Id.length; i++) {
                addKeyValue("strDiem_DanhSachHoc_Id", arrChecked_Id[i]);
            }
        });
        edu.system.getList_MauImport("zonebtnBaoCao_DiemNgoai", function (addKeyValue) {
            var obj_list = {
                'strTuKhoa': edu.util.getValById('txtSearch_TuKhoa'),
                'strDaoTao_LopQuanLy_Id': edu.util.getValById('dropSearch_LopQuanLy'),
                'strChucNang_Id': edu.system.strChucNang_Id,//'8D42F005D1934295959197C5278F4BF9',
        ...
        $("#btnXacNhanDS").click(function () {
            $("#modal_XacNhanDS").modal("show");
            $(".loaiXacNhan").html("Xác nhận");
            me.strLoaiXacNhan = "XACNHAN_HOANTHANH_NHAP";
            //var strLopHocPhan_Id = me.dtLichHoc.find(e => e.IDLICHHOC === me.strLichHoc_Id).IDLOPHOCPHAN;
            var strDSDiem_Id = me.dtDSDiem.length ? me.dtDSDiem[0].ID : "";
            me.getList_BtnXacNhanSanPham("#zoneBtnXacNhanDS", me.strLoaiXacNhan);
        });

        $("#btnXacNhanTP").click(function () {
            $("#modal_XacNhanDS").modal("show");
            $(".loaiXacNhan").html("Xác nhận");
            me.strLoaiXacNhan = "XACNHAN_HOANTHANH_DIEMDANH";
            //var strLopHocPhan_Id = me.dtLichHoc.find(e => e.IDLICHHOC === me.strLichHoc_Id).IDLOPHOCPHAN;
        ...
        $("#zoneBtnXacNhanDS").delegate('.btnxacnhan', 'click', function () {
            var strTinhTrang = this.id; 
            var strMoTa = edu.util.getValById("txtNoiDungXacNhanSanPham");
            me.save_XacNhanSanPhamDS("", strTinhTrang, strMoTa, me.strLoaiXacNhan); 
        });

        $(".btnXacNhanTungSinhVien").click(function () {
            $("#modal_XacNhanTungSinhVien").modal("show");
            me.strLoaiXacNhan = "XACNHAN_HOANTHANH_NHAP_NGUOIHOC";
            me.getList_BtnXacNhanSanPham("#zoneBtnXacNhanTungSinhVien", me.strLoaiXacNhan);
            me.actionCopyXacNhan("tblNhapDiem", [1, 2, 3]);
            var point = document.getElementById("tblNhapDiem").getElementsByTagName("tbody")[0].rows
            for (var i = 0; i < point.length; i++) {
                me.getList_XacNhan(me.strDSDiem_Id + point[i].id, "", data => {
                    if (data.length > 0) $("#lblTinhTrang" + data[0].QLSV_NGUOIHOC_ID).html(data[0].TEN)
                }, me.strLoaiXacNhan)
            }
        });

        $(".btnXacNhanTungSinhVienDD").click(function () {
            $("#modal_XacNhanTungSinhVien").modal("show");
            me.strLoaiXacNhan = "XACNHAN_HOANTHANH_DIEMDANH_NGUOIHOC";
            me.getList_BtnXacNhanSanPham("#zoneBtnXacNhanTungSinhVien", me.strLoaiXacNhan);
            me.actionCopyXacNhan("tblNhapDiem", [1, 2, 3]);
            var point = document.getElementById("tblNhapDiem").getElementsByTagName("tbody")[0].rows
            for (var i = 0; i < point.length; i++) {
        ...
        $("#zoneBtnXacNhanTungSinhVien").delegate('.btnxacnhan', 'click', function () {
            ...
            if (arrChecked_Id.length == 0) {
                ...
                arrChecked_Id.forEach(e => {
                    me.save_XacNhanSanPham(me.strDSDiem_Id + e, strTinhTrang, strMoTa, me.strLoaiXacNhan);
                })
    ...
    getList_Hoc: function (strDeTai_Id) {
        ...
        //var obj_save = {
        //    'action': 'SV_XuLyDiem_MH/DSA4BRIFKCQsHgUgLykSICIpCS4i',
        //    'func': 'pkg_diem_xulydiem.LayDSDiem_DanhSachHoc',
        //    'iM': edu.system.iM,
        //    'strTuKhoa': edu.util.getValById('txtSearch_TuKhoa'),
        //    'strDaoTao_LopQuanLy_Id': edu.util.getValById('dropSearch_LopQuanLy'),
        //    'strChucNang_Id': edu.system.strChucNang_Id,//'8D42F005D1934295959197C5278F4BF9',
    ...
    genTable_Hoc: function (data, iPager) {
        ...
        var jsonForm = {
            ...
            aoColumns: [
                ...
                {
                    "mRender": function (nRow, aData) {
                        return aData.XACNHANHOANTHANHNHAPDIEM == 1 ? "Đã xác nhận": "";
                    }
                },
                {
                    "mRender": function (nRow, aData) {
                        return aData.XACNHANHOANTHANHDIEMDANH == 1 ? "Đã xác nhận" : "";
                    }
                },
                {
                    "mRender": function (nRow, aData) {
    ...
    genHTML_ThongTinHienThiCot: function (data, iPager, strCongThuc) {
        ...
        row += '<th><input type="checkbox" class="chkSystemSelectAll"></th>';
        row += '</tr>';
        //row += '<tr>';
    ...
    genTable_NguoiHoc: function (data, iPager) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            row += '<td class="td-center"><input type="checkbox" id="checkX' + data[i].ID + '"/></td>';
    ...
    getList_NguoiHoc_Diem: function (rowNguoiHoc, strMaCot, dChiXem) {
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    if (dChiXem != 1) {
                        $("#" + strNguoiHoc_Id + "_" + strMaCot).html('<input class="inputNhapDiem" id="input' + strNguoiHoc_Id + "_" + strMaCot + '" value="' + data.Id + '" name="' + data.Id + '" style="width: 100%" />');
                    } else {
                    ...
                    for (var i = 0; i < data.Data.length; i++) {
                       ...
                       // if (strNguoiHoc_Id == "A1C0DD8A83BA44439246135C0A3536D1" || strNguoiHoc_Id == "16498D9C68664FFC9693F78B4B4C0CC2") console.log(data.Data);
                        ...
                        if (dCellChiXem == 1) {
                            ...
                            $("#" + strNguoiHoc_Id + "_" + strMaCot).html('<input class="inputNhapDiem" id="input' + strNguoiHoc_Id + "_" + strMaCot + '" value="' + dGiaTri + '" name="' + dGiaTri + '" style="width: 100%"/>');
                        }
                        //if (x.length > 0) {
    ...
    save_XacNhanSanPhamDS: function (strSanPham_Id, strTinhTrang_Id, strNoiDung, strLoaiXacNhan) {
        var me = this;
        var obj_save = {
            'action': 'D_NguoiHoc/XacNhanDiem_DanhSachHoc',
            'type': 'POST',
    ...
    actionCopyXacNhan: function (strTable_Id, arrCols) {
        ...
        $("#tblModal_DanhSachXacNhan").delegate('.btnChiTiet', 'click', function (e) {
            ...
            if (edu.util.checkValue(strId)) {
                if (strId) {
                    me.getList_XacNhan(me.strDSDiem_Id + strId, "tblModal_XacNhanDSTungSinhVien", null, me.strLoaiXacNhan)
                }
            }
            else {
    ...
    getList_KeHoachDKH: function () {
        var me = this;
        var obj_save = {
            'action': 'XLHV_TP_ToChucThi_MH/DSA4BRIFIC8mCjgeCiQJLiAiKQUgLyYKOAPP',
            'func': 'pkg_thi_tochucthi.LayDSDangKy_KeHoachDangKy',
            'iM': edu.system.iM,
            ...
            success: function (data) {
                if (data.Success) {
                    edu.system.loadToCombo_data({
                        ...
                        renderPlace: ["dropSearch_KeHoachDKH"],
                        type: "",
                        title: "Chọn kế hoạch đăng ký học",
                    })
                }
                else {
