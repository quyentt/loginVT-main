...
--Note:
----------------------------------------------*/
function DieuKienXetNangLuong() { };
DieuKienXetNangLuong.prototype = {
    strKeHoachXetNangLuong_Id: '',
    strDieuKienXetNangLuong_Id: '',
    dtDieuKienXetNangLuong: [],
    dtDMThanhPhan: [],
    strDMThanhPhan_Id: '',
    strTenDanhMuc_Id: '',
    ...
    init: function () {
        ...
        edu.system.loadToCombo_DanhMucDuLieu("LUONG.LOAIXETNANGLUONG", "dropLoaiXetLuong");
        /*------------------------------------------
        ...
        $("#btnLuuEditHienThi").click(function () {
            me.save_DieuKienXetNangLuong();
        });
        $(".btnAddKeHoachXetNangLuong").click(function () {
            me.toggle_edit();
        });
        $("#tblKeHoachXetNangLuong").delegate('.btnChonDanhSach', 'click', function () {
            var strDanhSach_Id = this.id;
            me.strKeHoachXetNangLuong_Id = strDanhSach_Id;
            me.toggle_detail()
        });

        $("#tblDieuKienXetNangLuong").delegate('.btnEdit', 'click', function () {
            me.viewForm_DieuKienXetNangLuong(this.id)
        });
        $("#tblDieuKienXetNangLuong").delegate(".btnDelete", "click", function () {
            var strId = this.id;
            console.log(this.id);
            strId = edu.util.cutPrefixId(/delete/g, strId);
            if (edu.util.checkValue(strId)) {
                edu.system.confirm(edu.constant.getting("NOTIFY", "CF_DELETE"));
                edu.util.setOne_BgRow(strId, "tblDieuKienXetNangLuong");
                $("#btnYes").click(function (e) {
                    me.delete_DieuKienXetNangLuong(strId);
                });
            }
            else {
        ...
        edu.system.loadToCombo_DanhMucDuLieu("NHANSU.THANHPHANLUONG", "dropNL_ThanhPhan", "", me.genTable_DMThanhPhan);
        ...
        $("#dropLoaiBangLuong").on("select2:select", function () {
            me.getList_DieuKienXetNangLuong();
        });
    },
    ...
    toggle_edit: function (strDanhSach_Id) {
        ...
        edu.util.viewValById("txtNL_ThuTu", "");
        edu.util.viewValById("txtNL_CongThuc", "");
        edu.util.viewValById("dropThanhPhanCha", "");
        edu.util.viewValById("dropNL_ThanhPhan", "");
        edu.util.viewValById("txtNL_KyHieu", "");
        me.strDieuKienXetNangLuong_Id = "";
    },
    /*------------------------------------------
    --Discription: [3] AccessDB HOC
    --ULR:  Modules
    -------------------------------------------*/
    getList_KeHoachXetNangLuong: function (strDeTai_Id) {
        var me = this;
        //--Edit
        var obj_list = {
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    me.genTable_KeHoachXetNangLuong(dtReRult, data.Pager);
                }
                else {
    ...
    genTable_KeHoachXetNangLuong: function (data, iPager) {
        var me = DieuKienXetNangLuong;
        $("#lblKeHoachXetNangLuong_Tong").html(iPager);
        //edu.util.viewHTMLById("lblKeHoachXetNangLuong_Tong", iPager);
        var jsonForm = {
        ...
        if (data != null && data.length == 1) {
            me.strKeHoachXetNangLuong_Id = data[0].ID;
            me.toggle_detail();
        }
    },
    /*------------------------------------------
    ...
    --ULR:  Modules
    -------------------------------------------*/
    save_DieuKienXetNangLuong: function () {
        var me = this;
        var obj_notify = {};
        //--Edit
        var obj_save = {
            'action': 'L_XetLuong_CauTruc/ThemMoi',
            

            'strId': "",
            'strNhanSu_KeHoachXetLuong_Id': me.strKeHoachXetNangLuong_Id,
            'strNhanSu_BangQuyDinh_Id': "",
            'strThanhPhan_Id': edu.util.getValById('dropNL_ThanhPhan'),
            'strThanhPhan_Cha_Id': edu.util.getValById('dropThanhPhanCha'),
            'iThuTu': edu.util.getValById('txtNL_ThuTu'),
            'strXauCongThucTinh': edu.util.getValById('txtNL_CongThuc'),
            'strKyHieu': edu.util.getValById('txtNL_KyHieu'),
            'strNguoiThucHien_Id': edu.system.userId,
        };
        if (edu.util.checkValue(me.strDieuKienXetNangLuong_Id)) {
            obj_save.action = 'L_XetLuong_CauTruc/CapNhat';
            obj_save.strId = me.strDieuKienXetNangLuong_Id;
        }
        //default
        edu.system.makeRequest({
            success: function (data) {
    ...
    getList_DieuKienXetNangLuong: function (strDanhSach_Id) {
        var me = this;
        //--Edit
        var obj_list = {
            'action': 'L_XetLuong_CauTruc/LayDanhSach',
            'strNhanSu_KeHoachXetLuong_Id': me.strKeHoachXetNangLuong_Id,
            'strNguoiThucHien_Id': "",
        };
        //
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    var dtReRult = data.Data;
                    me.genTable_DieuKienXetNangLuong(dtReRult, data.Pager);
                    me.genCombo_ThanhPhan_Cha(dtReRult);
                    me.insertHeaderTable("tblViewKeHoachXetNangLuong", dtReRult, null);
                    me.dtDieuKienXetNangLuong = dtReRult;
                }
                else {
                    edu.system.alert(obj_list + " : " + data.Message, "s");
                }
    ...
    delete_DieuKienXetNangLuong: function (Ids) {
        var me = this;
        //--Edit
        var obj_delete = {
            'action': 'L_XetLuong_CauTruc/Xoa',
    ...
    --ULR:  Modules
    -------------------------------------------*/
    genTable_DieuKienXetNangLuong: function (data, iPager) {
        var me = this;
        var jsonForm = {
            strTable_Id: "tblDieuKienXetNangLuong",
            aaData: data,
            colPos: {
                center: [0, 4, 5]
            },
    ...
    viewForm_DieuKienXetNangLuong: function (strId) {
        var me = this;
        //call popup --Edit
        var data = edu.util.objGetDataInData(strId, me.dtDieuKienXetNangLuong, "ID")[0];
        me.toggle_edit();
        //view data --Edit
        edu.util.viewValById("txtNL_ThuTu", data.THUTU1);
        edu.util.viewValById("txtNL_CongThuc", data.XAUCONGTHUCTINH);
        edu.util.viewValById("dropThanhPhanCha", data.THANHPHAN_CHA_ID);
        edu.util.viewValById("dropNL_ThanhPhan", data.THANHPHAN_ID);
        edu.util.viewValById("txtNL_KyHieu", data.KYHIEU);
        edu.util.viewValById("dropNL_TinhThue", data.THUNHAPTINHTHUE);
        me.strDieuKienXetNangLuong_Id = data.ID;
    },
    ...
    getList_CauTruc_TuKhoa: function (strDeTai_Id) {
        ...
        //--Edit
        var obj_list = {
            'action': 'L_XetLuong_TuKhoa/LayDanhSach',
    ...
    genTable_DMThanhPhan: function (data, iPager) {
        var me = main_doc.DieuKienXetNangLuong;
    ...
    genCombo_LoaiBangLuong: function (data) {
        var me = this;
        var strBangLuongChing_Id = edu.util.objGetDataInData("BLC", data, "MA")[0]["ID"];
        var obj = {
            data: data,
            renderInfor: {
                id: "ID",
                parentId: "",
                name: "TEN",
                default_val: strBangLuongChing_Id
            },
