...
ThamNien.prototype = {
    ...
    init: function () {
        ...
        $("#btnSave_ThamNien").click(function () {
            ...
            for (var i = 0; i < x.length; i++) {
                ...
                me.save_ThamNien(strNhanSu_Id);
            }
        });

        $("#Save_NS_ThamNien").click(function () {
            me.update_ThamNien();
        });
        ...
        $("[id$=chkSelectAll_ThamNien]").on("click", function () {
            edu.util.checkedAll_BgRow(this, { table_id: "tblThamNien" });
        });
        $("#btnXoaThamNien").click(function () {
            var arrChecked_Id = edu.util.getArrCheckedIds("tblThamNien", "checkHS");
            ...
            $("#btnYes").click(function (e) {
                ...
                for (var i = 0; i < arrChecked_Id.length; i++) {
                    me.delete_ThamNien(arrChecked_Id[i]);
                }
            });
        });
        $("#tblThamNien").delegate(".btnEdit", "click", function () {
            var strId = this.id;
            strId = edu.util.cutPrefixId(/edit/g, strId);
            edu.util.setOne_BgRow(strId, "tblThamNien");
            if (edu.util.checkValue(strId)) {
                me.strThamNien_Id = strId;
                me.editForm_NhanSu_ThamNien(me.dtThamNien.find(e => e.ID === strId));
            }
            else {
        ...
        edu.system.getList_MauImport("zonebtnBaoCao_CSAP", function (addKeyValue) {
            var obj_list = {
                'strTuKhoa': edu.util.getValById("txtSearch_ThamNien_TuKhoa"),
                'strDaoTao_CoCauToChuc_Id': edu.util.getValById("dropSearch_DonViThanhVien"),
                'strNgayPhatSinh_TuNgay': edu.util.getValById("txtSearch_ThamNien_TuNgay"),
                'strNgayPhatSinh_DenNgay': edu.util.getValById("txtSearch_ThamNien_DenNgay"),
                'strSearch_NhanSu_HoSoCanBo_Id': edu.util.getValById("dropSearch_ThanhVien"),
                'strNam': edu.util.getValById('txtSearch_Nam'),
                'dLaCanBoNgoaiTruong': edu.util.getValById('dropSearch_LaCanBo_ThamNien'),
    ...
    page_load: function () {
        ...
        me.getList_HS();
        //edu.system.loadToCombo_DanhMucDuLieu("NHANSU.LUONG.PHANLOAILUONGAPDUNG", "dropSearch_PhanLoai,dropPhanLoai");
    },
    ...
    --ULR: Modules
    -------------------------------------------*/
    getList_ThamNien: function () {
        var me = this;
        //--Edit
        var obj_list = {
            'action': 'NS_QT_ThamNien/LayDanhSach',
            'type': 'GET',
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    me.dtThamNien = data.Data;
                    me.genTable_ThamNien(data.Data, data.Pager);
                }
                else {
    ...
    save_ThamNien: function (strNhanSu_Id) {
        var me = this;
        var obj_save = {
            'action': 'NS_QT_ThamNien/ThemMoi',
            'type': 'POST',
            ...
            'strTuNgay': edu.util.getValById('txtTuNgay' + strNhanSu_Id),
            'strDenNgay': edu.util.getValById('txtDenNgay' + strNhanSu_Id),
            'strNoiDung': edu.util.getValById('txtAAAA'),
            'strNguoiThucHien_Id': edu.system.userId,
        };
        if (obj_save.strId) obj_save.action = 'NS_QT_ThamNien/CapNhat';
        //default
    ...
    update_ThamNien: function () {
        var me = this;
        var objNS = me.dtThamNien.find(e => e.ID === me.strThamNien_Id);
        var obj_save = {
            'action': 'NS_QT_ThamNien/ThemMoi',
            'type': 'POST',
            'strId': me.strThamNien_Id,
    ...
    delete_ThamNien: function (strIds) {
        var me = this;
        //--Edit
        var obj_delete = {
            'action': 'NS_QT_ThamNien/Xoa',
    ...
    --ULR: Modules
    -------------------------------------------*/
    genTable_ThamNien: function (data, iPager) {
        var me = this;
        $("#lblHSLL_ThamNien_Tong").html(iPager);
        var jsonForm = {
            strTable_Id: "tblThamNien",
            aaData: data,
            bPaginate: {
                strFuntionName: "main_doc.ThamNien.getList_ThamNien()",
                iDataRow: iPager
            },
            colPos: {
                center: [0, 4, 5, 6, 7]
    ...
    editForm_NhanSu_ThamNien: function (data) {
        var me = this;
        edu.util.toggle_overide("zone-bus", "zoneEdit_NhanSu_LuongThamNien");
        //view data --Edit
    ...
    genHTML_NhanSu: function (strNhanSu_Id, bcheckadd) {
        ...
        html += "<td><input class='form-control input-datepicker' id='txtTuNgay" + strNhanSu_Id + "' placeholder='dd/mm/yyyy' /></td>";
        html += "<td><input class='form-control input-datepicker' id='txtDenNgay" + strNhanSu_Id + "' placeholder='dd/mm/yyyy' /></td>";
    ...
    cbGetListModal_NhanSu: function (data, iPager) {
        ...
        var jsonForm = {
            ...
            aaData: data,
            bPaginate: {
                strFuntionName: "main_doc.ThamNien.getList_NhanSu('SEARCH')",
                iDataRow: iPager,
            },
            colPos: {
                left: [2],
