...
BangLuong.prototype = {
    ...
    insertHeaderTable: function (strTable_Id, obj, strQuanHeCha) {
        //Khởi tạo table
        $("#" + strTable_Id).html('<thead style="font-weight: bold"><tr><td>Stt</td><td>Mã số</td><td>Họ tên</td><td>CCTC</td></tr><tr></tr><tr></tr><tr></tr><tr></tr><tr></tr><tr></tr><tr></tr></thead><tbody></tbody><tfoot></tfoot>');
    ...
    getList_DuLieuBangLuong: function () {
        ...
        var obj_list = {
            'action': 'L_DuLieuBangLuong/LayDanhSach',
            

            'strDaoTao_CoCauToChuc_Id': edu.util.getValById('dropSearch_DonViThanhVien'),
            'strNhanSu_HoSoCanBo_Id': edu.util.getValById('dropSearch_ThanhVien'),
            'dThang': edu.util.getValById('txtSearch_Thang'),
    ...
    genData_DuLieuBangLuong: function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            var aData = data[i];
            $("#lbl" + aData.NHANSU_HOSOCANBO_ID + "_" + aData.THANHPHAN_ID).html(edu.util.formatCurrency(aData.THANHPHAN_GIATRI));
        }
        var arrSum = [];
        for (var i = 0; i < me.arrHeadDiem_Id.length; i++) {
            arrSum.push(i + 4);
        }
        edu.system.insertSumAfterTable("tblKhoiTao_NhanSu", arrSum);
    },
    /*------------------------------------------
