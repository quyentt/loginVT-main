...
ThuTien.prototype = {
    ...
    init: function () {
        ...
        $("#btnSave_ThuTien").click(function (e) {
            e.stopImmediatePropagation();
            $("#btnSave_ThuTien").hide();
    ...
    cbGenTable_NguoiHoc_TTTS: function (data, iPager) {
        ...
        var jsonForm = {
            ...
            aaData: data,
            bPaginate: {
                strFuntionName: "edu.extend.getList_NguoiHoc_TTTS(main_doc.ThuTien.iTinhTrangNhapHoc,main_doc.ThuTien.strKeHoach_Id, '" + strTuKhoa + "',main_doc.ThuTien.cbGenTable_NguoiHoc_TTTS)",
                iDataRow: iPager,
            },
            ...
            aoColumns: [
                ...
                , {
                    "mRender": function (nRow, aData) {
                        ...
                        html += '<span><a id="' + aData.ID + '" class="" href="#">Chọn</a></span>';
                        if (iDaNhapHoc == 1) {
    ...
    checkAuto_Select_NguoiHoc_TTTS: function (data) {
        ...
        if (data.length == 1) {//only one record
            ...
            me.checkCondition_ThuTien(me.dtNguoiHoc_Print);
            //return new Promise(function (resolve, reject) {
            //    me.getDetail_NguoiHoc_TTTS(strNguoiHoc_Id, main_doc.ThuTien.dtNguoiHoc, resolve, reject);
                ...
                , {// so tien dieu chinh
                    "mRender": function (nRow, aData) {
                        ...
                        var html = '<input type="text" id="txtKhoanThu_SoTien_Edit' + strId + '" value="' + edu.util.formatCurrency(dSoDaThu) +
                            '" onblur="main_doc.ThuTien.sumEdit_SoTien_PhieuDaThu()" class="form-control td-right" data-ax5formatter="money" />';
                        return html;
                    }
                }, {//so tien da thu
                    "mRender": function (nRow, aData) {
            ...
            edu.system.makeRequest({
                success: function (d, s, x) {
                    if (d.Success) {
                        ...
                        edu.extend.getData_Phieu(d.Id, "HOADON", "MauInHoaDon", main_doc.ThuTien.changeWidthPrint);
