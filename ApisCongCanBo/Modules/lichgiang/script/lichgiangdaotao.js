...
--Note:
----------------------------------------------*/
function LichGiangDaoTao() { };
LichGiangDaoTao.prototype = {
    strThongTin_Id: '',
    dtThongTin: [],
    iNgayBatDau: 0,
    strGiangVien_Id: '',
    ...
    init: function () {
        ...
        edu.system.loadToCombo_DanhMucDuLieu("TKB.LICHGIANG.XACNHANDOILICH", "dropSearch_KetQuaXuLy,dropPhetDuyet_TrangThai2,dropPhetDuyet_TrangThai");
        me.getList_HocKy();
        me.getList_HocPhan();
        me.getList_NguoiGui();
        me.getList_LGXacNhan();
        //me.getList_TrangThaiXacNhan();
        me.getList_KhoaChuyenMon();
        ...
        $("#btnPheDuyet").click(function () {
            ...
            me.getList_XacNhan(me.strDoiLich_Id, "tblModal_XacNhan", null, 'XACNHAN_HOANTHANH_NHAP');
        });
        $("#btnLuuPheDuyet").click(function () {
            edu.system.confirm("Bạn có chắc chắn phê duyệt không?");
            $("#btnYes").click(function (e) {
                me.save_XacNhan(me.strDoiLich_Id, $("#dropPhetDuyet_TrangThai").val());
            });
        });
        $("#btnLuuPheDuyet2").click(function () {
            ...
            arrChecked_Id.forEach(e => {
                me.save_XacNhan(e, $("#dropPhetDuyet_TrangThai2").val())
            })
            //edu.system.confirm("Bạn có chắc chắn phê duyệt không?");
            //$("#btnYes").click(function (e) {
            //    me.save_PhetDuyet(me.strDoiLich_Id);
        ...
        edu.system.getList_MauImport("zonebtnBaoCao_DuyetLichGiang", function (addKeyValue) {
           
            addKeyValue("strTuKhoa", edu.util.getValById('txtAAAA'));
            addKeyValue("strDaoTao_KhoaQuanLy_Id", edu.util.getValById('dropSearch_KhoaQuanLy'));
            addKeyValue("strDaoTao_ThoiGianDaoTao_Id", edu.util.getValById('dropSearch_HocKy'));
            addKeyValue("strNgayGuiYeuCau_TuNgay", edu.util.getValById('txtSearch_TuNgay'));
            addKeyValue("strNgayGuiYeuCau_DenNgay", edu.util.getValById('txtSearch_DenNgay'));
            addKeyValue("strDaoTao_HocPhan_Id", edu.util.getValById('dropSearch_HocPhan'));
            addKeyValue("strNguoiGuiYeuCau_Id", edu.util.getValById('dropSearch_NguoiGui'));
            addKeyValue("strTrangThaiDuyet_Id", edu.util.getValById('dropSearch_TrangThaiDuyet'));
            addKeyValue("strTrangThaiXuLy_Id", edu.util.getValById('dropSearch_KetQuaXuLy'));
    ...
    getList_LGXacNhan: function (strDanhSach_Id) {
        var me = this;
        var obj_list = {
            'action': 'KHCT_LichGiang_DoiLich/LayDSLichGiang_Doi_PhamVi_XL',
            'type': 'GET',
    ...
    genTable_LGXacNhan: function (data, iPager) {
        ...
        var jsonForm = {
            ...
            bPaginate: {
                strFuntionName: "main_doc.LichGiangDaoTao.getList_LGXacNhan()",
                iDataRow: iPager,
            },
            aaData: data,
            colPos: {
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    me.ThucHienGuiXacNhan(strSanPham_Id);
                }
                else {
                    obj_notify = {
                        type: "w",
    ...
    getList_XacNhan: function (strSanPham_Id, strTable_Id) {
        var me = this;
        var obj_list = {
            'action': 'KHCT_LichGiang_DoiLich/LayDSTKB_XacNhanDoiLich',
            'type': 'GET',
            'strTuKhoa': edu.util.getValById('txtAAAA'),
            'strNguoiThucHien_Id': edu.system.userId,
            'strsanpham_Id': strSanPham_Id,
    ...
    getList_HocPhan: function (strId) {
        ...
        var obj_list = {
            'action': 'KHCT_LichGiang_DoiLich/LayDSHocPhanDuyetKhoaQuanLy',
            'type': 'GET',
            'strChucNang_Id': edu.system.strChucNang_Id,
            'strDaoTao_ThoiGianDaoTao_Id': edu.util.getValById('dropSearch_HocKy'),
            'strDaoTao_KhoaQuanLy_Id': edu.util.getValById('dropSearch_KhoaQuanLy'),
    ...
    getList_NguoiGui: function (strId) {
        ...
        var obj_list = {
            'action': 'KHCT_LichGiang_DoiLich/LayDSNguoiGuiDuyetKhoaQuanLy',
            'type': 'GET',
            'strChucNang_Id': edu.system.strChucNang_Id,
            'strDaoTao_ThoiGianDaoTao_Id': edu.util.getValById('dropSearch_HocKy'),
            'strDaoTao_KhoaQuanLy_Id': edu.util.getValById('dropSearch_KhoaQuanLy'),
    ...
    getList_KhoaChuyenMon: function (strId) {
        var me = this;
        //--Edit

        var obj_list = {
            'action': 'KHCT_LichGiang_DoiLich/LayDSKhoaQuanLyChuyenMon',
            'type': 'GET',
            'strChucNang_Id': edu.system.strChucNang_Id,
            'strDaoTao_ThoiGianDaoTao_Id': edu.util.getValById('dropSearch_HocKy'),
            'strNguoiThucHien_Id': edu.system.userId,
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    var dtReRult = data.Data;
                    me.genCombo_KhoaChuyenMon(dtReRult);
                }
                else {
                    edu.system.alert(data.Message, "s");
                }
    ...
    genCombo_KhoaChuyenMon: function (data) {
        var obj = {
            data: data,
            renderInfor: {
                id: "ID",
    ...
    ThucHienGuiXacNhan: function (strSanPham_Id) {
        var me = this;
        var obj_list = {
            'action': 'CMS_TienIch/ThucHienGuiEmailTheoCauTruc',
            'type': 'GET',            
            'strCauTrucGuiEmailId': "PDT_XACNHAN_THAYDOILICHGV",
            'strDuLieuId': strSanPham_Id,
