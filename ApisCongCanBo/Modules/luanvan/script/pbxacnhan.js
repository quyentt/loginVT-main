...
PBXacNhan.prototype = {
    ...
    init: function () {
        ...
        me.getList_KeHoach();
        me.getList_CanBo();
        edu.system.loadToCombo_DanhMucDuLieu("PB.TINHTRANG.XACNHAN.TRUOCKHILAPHOIDONG", "dropTrangThaiXacNhan");
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    me.genTable_ThanhVien1(dtResult);
                }
                else {
                    edu.system.alert(obj_list.action + ": " + data.Message, "w");
                }
    ...
    genTable_ThanhVien1: function (data) {
        var me = this;
        $("#tblNguoiHuongDan1 tbody").html("");
        for (var i = 0; i < data.length; i++) {
            var strKetQua_Id = data[i].ID;
            console.log(strKetQua_Id);
            var aData = data[i];
            ...
            row += '<td style="text-align: center"></td>';
            row += '</tr>';
            $("#tblNguoiHuongDan1 tbody").append(row);
            me.genComBo_CanBo("dropCanBo" + strKetQua_Id, aData.NGUOIDUNG_ID);
            me.genComBo_VaiTro("dropVaiTro" + strKetQua_Id, aData.VAITRO_ID);
            edu.system.viewFiles("txtFileDinhKem" + strKetQua_Id, strKetQua_Id, "LVLA_Files");
        }
    ...
    save_XacNhanSanPham: function (strSanPham_Id, strTinhTrang_Id, strNoiDung, strLoaiXacNhan) {
        var me = this;
        var obj_save = {
            'action': 'LVLA_BV_Chung/Them_BV_XacNhan_PB',
            'type': 'POST',
    ...
    getList_XacNhan: function (strSanPham_Id, strTable_Id, callback, strLoaiXacNhan) {
        var me = this;
        var obj_list = {
            'action': 'LVLA_BV_Chung/LayDSBV_XacNhan_PB',
            'type': 'GET',
