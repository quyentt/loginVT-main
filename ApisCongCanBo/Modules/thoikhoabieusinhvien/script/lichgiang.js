...
LichGiang.prototype = {
    ...
    init: function () {
        ...
        me.genHtml_Month(0);
        // Vẽ bảng "Lớp học phần (không có lịch chi tiết)" ngay khi vào trang để người dùng luôn
        // thấy bảng + thông báo, thay vì màn hình trống không biết chuyện gì (2026-09-17)
    ...
    --Note: strGiangVien_Id trên trang này thực chất là QLSV_NguoiHoc_Id của sinh viên
            đang được tra cứu (xem action_NguoiDung), nên dùng API phía sinh viên.
    -------------------------------------------*/
    ...
    renderTable_TKBLopKhongCoLichChiTiet: function (data, strTrangThai, strThongBao) {
        ...
        // Bảng luôn hiển thị, kể cả rỗng — chỉ đổi nội dung tbody
        ...
        if (strTrangThai === "init") {
            $tbody.html(fnEmpty("fal fa-calendar-day", "#b8c0d4",
                "Nhập mã sinh viên và chọn một ngày trên lịch để xem lớp học phần không có lịch chi tiết"));
            return;
        }
    ...
    genTable_ThongTin: function (data, iPager) {
        ...
        data.forEach((e, nRow) => {
            ...
            html += '<div class="task task-1 ' + temp + '" id="' + e.ID + '" style="top:' + e.PHUTBATDAU + 'px; height: ' + iTop + 'px; background-color: ' + strMauNen + '; cursor: pointer">';
