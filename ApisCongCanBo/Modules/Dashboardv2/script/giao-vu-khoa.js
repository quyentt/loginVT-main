function DashboardGiaoVuKhoa() {
    ...
    this.updateDataByFilters = function (filters) {
        ...
        self.chartData.studentOutcomeChart = self.buildStudentOutcomeDistributionByCohort(filters);
    ...
    this.initCharts = function () {
        ...
        // Chart 3: Student outcome distribution by cohort (stacked 100%)
        var elOutcome = document.getElementById('studentOutcomeChart');
        if (elOutcome) {
            self.safeDestroy(self.charts.studentOutcomeChart);
            var optOutcome = self.getStudentOutcomeChartOptions();
            self.charts.studentOutcomeChart = new Chart(elOutcome.getContext('2d'), {
                type: 'bar',
                data: self.chartData.studentOutcomeChart,
                options: optOutcome
            });
        }
    ...
    this.ensureOutcomeLabelPlugin = function () {
        if (typeof Chart === 'undefined') return;
        if (Chart._gvkOutcomeLabelPluginRegistered) return;
        Chart._gvkOutcomeLabelPluginRegistered = true;

        Chart.plugins.register({
            afterDatasetsDraw: function (chartInstance, easing) {
                try {
                    var opts = chartInstance && chartInstance.config && chartInstance.config.options;
                    if (!opts || !opts._gvkDrawPercentLabels) return;
                    ...
                    for (var di = 0; di < datasets.length; di++) {
                        ...
                        for (var i = 0; i < meta.data.length; i++) {
                            var val = datasets[di].data[i];
                            var v = Number(val);
                            if (!isFinite(v)) continue;
                            // avoid clutter for tiny segments
                            if (v < 6) continue;
                            var bar = meta.data[i];
                            if (!bar || !bar._model) continue;
                            var x = bar._model.x;
                            // midpoint of the stacked segment
                            var y = (bar._model.y + bar._model.base) / 2;
    ...
    this.getStudentOutcomeChartOptions = function () {
        var opts = self.getChartOptions();
        opts._gvkDrawPercentLabels = true;
        ...
        opts.tooltips.callbacks = {
            title: function (items, data) {
                if (!items || !items.length) return '';
                var label = data.labels[items[0].index] || '';
                return 'Khóa: ' + label;
            },
            label: function (tooltipItem, data) {
                var ds = data.datasets[tooltipItem.datasetIndex] || {};
    ...
    this.buildStudentOutcomeDistributionByCohort = function (filters) {
        // Xác định theo A/B/C (như risk list) nhưng tổng hợp theo khóa.
        // Quy ước mô hình hoá (không overlap phức tạp):
        // - Nguy cơ học vụ (%) ~ C
        // - Chậm tiến độ (%) ~ max(A, B) nhưng không vượt quá 100 - C
        // - Bình thường (%) = phần còn lại
        var seedBase = self.hashString('outcome|' + JSON.stringify(filters || {}));
        var cohorts = ['K20', 'K21', 'K22'];
        ...
        for (var i = 0; i < cohorts.length; i++) {
            var cSeed = seedBase + self.hashString(cohorts[i]) + i * 41;
            var aPct = self.randBetween(cSeed + 1, 2, 22);
            var bPct = self.randBetween(cSeed + 2, 2, 18);
            var cPct = self.randBetween(cSeed + 3, 1, 12);

            // make K22 slightly riskier in mock to mirror sample shape
            if (cohorts[i] === 'K22') {
                aPct += 3;
                bPct += 2;
                cPct += 1;
            ...
            var riskPct = Math.max(0, Math.min(100, C.pct));
            var slowPct = Math.max(A.pct, B.pct);
            slowPct = Math.max(0, Math.min(100 - riskPct, slowPct));
            var normalPct = Math.max(0, 100 - riskPct - slowPct);

            // round while keeping sum=100
            var n0 = Math.round(normalPct);
            var s0 = Math.round(slowPct);
            var r0 = Math.round(riskPct);
            var diff = 100 - (n0 + s0 + r0);
            if (diff !== 0) {
                // adjust normal first, then slow
                n0 = Math.max(0, Math.min(100, n0 + diff));
            }
        ...
        return {
            ...
            datasets: [
                {
                    label: 'Bình thường / đúng lộ trình',
                    data: normal,
