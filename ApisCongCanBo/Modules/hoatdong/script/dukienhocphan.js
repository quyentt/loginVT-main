...
DuKienHocPhan.prototype = {
    ...
    init: function () {
        ...
        //    var html = '';
        //    data.forEach(aData => {
        //        html += '<th class="td-center">' + aData.TEN + '</th><th class="td-center">' + aData.TEN + '(Tăng/Giảm)</th>';
        //    })
        //    console.log(html);
        //    $("#tblHocPhanDuKien thead tr:eq(1)").append(html);
        //    document.getElementById("lblSLTheoTinhChat").colSpan = (data.length * 2);
        //});
        edu.system.loadToCombo_DanhMucDuLieu("KH.KHOA.HOCPHAN.XACNHAN", "dropLoaiXacNhan")
        //$("#dropSearch_LoaiXacNhan").on("select2:select", function () {
        //    me.getList_BtnXacNhan();
        //});
        $("#btnSearch").click(function () {
            if (!edu.util.getValById("dropKhoaQuanLy_CB")) {
                edu.system.alert("Bạn cần chọn khoa quản lý"); 
                return;
            }
        ...
        $("#btnAddHPTheoChuongTrinh").click(function () {
            if (!edu.util.getValById("dropSearch_KeHoachChiTiet")) {
                edu.system.alert("Bạn cần chọn kế hoạch");
                return;
            }
            $("#themhocphantheochuongtrinhdaotao").modal("show")
            //edu.util.toggle_overide("zone-bus", "themhocphantheochuongtrinhdaotao");
        ...
        $("#btnAddHPTheoDonVi").click(function () {
            ...
            $("#themhocphantheodonvi").modal("show")
            //edu.util.toggle_overide("zone-bus", "themhocphantheodonvi");
        ...
        $("#btnSave_HocPhanDonVi").click(function () {
            ...
            for (var i = 0; i < arrChecked_Id.length; i++) {
                ...
                me.save_HocPhanDV(aData.DAOTAO_HOCPHAN_ID, aData.DAOTAO_TOCHUCCHUONGTRINH_ID, aData.KH_NAM_CHITIET_ID);
            }
        });
        ...
        $("#btnLuuTangGiam").click(function () {
            var arrThem = [];
            var arrXoa = [];
            var x = $("#tblHocPhanDuKien tbody input.doituongchecktanggiam");
            for (var i = 0; i < x.length; i++) {
                if ($(x[i]).val() != edu.util.returnEmpty($(x[i]).attr("name"))) {
                    arrThem.push(x[i]);
                }
        ...
        $("#btnDelete_HocPhanDonVi").click(function () {
            ...
            $("#btnYes").click(function (e) {
                ...
                edu.system.genHTML_Progress("zoneprocessXXXX", arrChecked_Id.length);
                for (var i = 0; i < arrChecked_Id.length; i++) {
                    me.delete_HocPhanDV(arrChecked_Id[i]);
                }
            });
        });
        ...
        $("#btnDongYXacNhan").click(function () {
            ...
            arrChecked_Id.forEach(e => {
                me.save_XacNhanTN(e, strTinhTrang, strMoTa, me.strLoaiXacNhan);
            })
        ...
        $("#tblHocPhanDuKien").delegate(".btnEditHocKy", "click", function () {
            var strId = this.id;
            me.strDuKienHocPhan_Id = strId;
            let aData = me.dtHocPhanDuKien.find(e => e.ID == strId);
            console.log(aData);
            console.log(me.dtHocPhanDuKien);
            let arrHocKy = [];
            if (aData.DAOTAO_THOIGIANDAOTAO_ID) {
                if (aData.DAOTAO_THOIGIANDAOTAO_ID.indexOf(',')) {
                    arrHocKy = aData.DAOTAO_THOIGIANDAOTAO_ID.split(',')
                } else {
                    arrHocKy = [aData.DAOTAO_THOIGIANDAOTAO_ID]
                }
                me["strEditHocKy_Id"] = arrHocKy[0];
            }
    ...
    save_TangGiam: function (point) {
        var me = this;
        var arrId = point.id.split("_");
        var aData = me.dtHocPhanDuKien.find(e => e.ID == arrId[1]);
        //--Edit
        var obj_save = {
            'action': 'KHCT_HoatDong_ThongTin_MH/AiAxDykgNRUgLyYGKCAsCikuIAUkGTQgNQPP',
            'func': 'PKG_KEHOACH_HOATDONG_THONGTIN.CapNhatTangGiamKhoaDeXuat',
            'iM': edu.system.iM,
            'strKH_HocPhan_DuKien_Id': aData.ID,
            'dTangGiamKhoaDeXuat': $(point).val() ? $(point).val() : undefined,
    ...
    genTable_HocPhanDuKien: function (data, iPager) {
            ...
            aoColumns: [
                ...
                },
                {
                    ...
                    "mRender": function (nRow, aData) {
                        let temp = aData.THOIGIAN ? aData.THOIGIAN : "Sửa";
        ...
        //            "mRender": function (nRow, aData) {
        //                var iThuTu = edu.system.icolumn++;
        //                return '<input id="txtTinhChat_' + aData.ID + '_' + main_doc.DuKienHocPhan.dtTinhChat[iThuTu].ID + '" class="form-control doituongcheck" />';
        //            }
        //        })
        //})
        jsonForm.aoColumns.push(
            {
                "mDataProp": "SOSVCHUAHOANTHANH"
            },
            {
                "mDataProp": "SOSVDUDKHOC"
            },
            {
            ...
            {
                "mRender": function (nRow, aData) {
                    return '<input id="txtTangGiam_' + aData.ID + '" class="form-control doituongchecktanggiam" value="' + edu.util.returnEmpty(aData.TANGGIAMKHOADEXUAT) + '" name="' + edu.util.returnEmpty(aData.TANGGIAMKHOADEXUAT) + '" />';
                }
            },
            {
    ...
    delete_HocPhanDV: function (Ids) {
        var me = this;
        //--Edit
        var obj_delete = {
            'action': 'KHCT_HoatDong_ThongTin_MH/GS4gHgoJHgkuIhEpIC8eBSQZNCA1',
            'func': 'PKG_KEHOACH_HOATDONG_THONGTIN.Xoa_KH_HocPhan_DeXuat',
            'iM': edu.system.iM,
            'strId': Ids,
            'strChucNang_Id': edu.system.strChucNang_Id,
            'strNguoiThucHien_Id': edu.system.userId
    ...
    save_HocPhanDV: function (strDaoTao_HocPhan_Id, strPhamViApDung_Id, strKH_Nam_ChiTiet_Id) {
        var me = this;
        //--Edit
        var obj_save = {
            'action': 'KHCT_HoatDong_ThongTin_MH/FSkkLB4KCR4JLiIRKSAvHgUkGTQgNQPP',
            'func': 'PKG_KEHOACH_HOATDONG_THONGTIN.Them_KH_HocPhan_DeXuat',
            'iM': edu.system.iM,
            'strKH_Nam_ChiTiet_Id': edu.system.getValById('dropSearch_KeHoachChiTiet'),
    ...
    save_XacNhanTN: function (strSanPham_Id, strTinhTrang_Id, strNoiDung) {
        var me = this;
        var obj_save = {
            'action': 'KHCT_HoatDong_XacNhan_MH/FSkkLB4KCR4KKS4gHgkRHhkgIg8pIC8P',
            'func': 'PKG_KEHOACH_HOATDONG_XACNHAN.Them_KH_Khoa_HP_XacNhan',
            'iM': edu.system.iM,
            'strSanPham_Id': strSanPham_Id,
            'strNguoiXacnhan_Id': edu.system.userId,
            'strThongTinXacNhan': strSanPham_Id,
    ...
    getList_XacNhanTN: function (strHoSoDuTuyen_Id, strTable_Id, callback) {
        var me = this;
        var obj_save = {
            'action': 'KHCT_HoatDong_XacNhan_MH/DSA4BRIKCR4KKS4gHgkRHhkgIg8pIC8P',
            'func': 'PKG_KEHOACH_HOATDONG_XACNHAN.LayDSKH_Khoa_HP_XacNhan',
            'iM': edu.system.iM,
    ...
    getList_EditHocKy: function () {
        var me = this;
        var obj_save = {
            'action': 'KHCT_HoatDong_ThongTin_MH/DSA4BRIVKS4oBiggLxUpJC4P',
            'func': 'PKG_KEHOACH_HOATDONG_THONGTIN.LayDSThoiGianTheo',
            'iM': edu.system.iM,
            'strKH_HocPhan_DuKien_Id': me.strDuKienHocPhan_Id,
            'strNguoiThucHien_Id': edu.system.userId,
        };
    ...
    genCombo_EditHocKy: function (data) {
        ...
        var obj = {
            ...
            renderInfor: {
                ...
                parentId: "",
                name: "THOIGIAN",
                default_val: me.strEditHocKy_Id
            },
    ...
    save_EditHocKy: function (strDaoTao_ThoiGian_KH_Id) {
        var me = this;
        var obj_save = {
            'action': 'KHCT_HoatDong_ThongTin_MH/EjQgHgoJHgkuIhEpIC8eBTQKKCQv',
            'func': 'PKG_KEHOACH_HOATDONG_THONGTIN.Sua_KH_HocPhan_DuKien',
            'iM': edu.system.iM,
            'strDaoTao_ThoiGianDaoTao_Id': edu.system.getValById('dropEditHocKy'),
            'strNguoiThucHien_Id': edu.system.userId,
            'strKH_HocPhan_DuKien_Id': me.strDuKienHocPhan_Id,
        };
        edu.system.makeRequest({
