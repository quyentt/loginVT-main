...
----------------------------------------------*/
function HuongNghienCuuChinh() { };
HuongNghienCuuChinh.prototype = {
    strCommon_Id: '',
    tab_actived: [],
    tab_item_actived: [],
    ...
    init: function () {
            ...
            if (!check) {
                ...
                switch (target) {
                    case "#tab_1": //Tieu su ban than
                        me.open_Collapse("key_huongnghiencuuchinh");
                        break;
        ...
        -------------------------------------------*/
        $("#btnSaveRe_HNCC").click(function () {
            me.save_HNCC();
            setTimeout(function () {
                me.resetPopup_HNCC();
            }, 1000);
        });
        $("#btnSave_HNCC").click(function () {
            me.save_HNCC();
        });
        $("#tbl_HNCC").delegate(".btnEdit", "click", function () {
            var strId = this.id;
            strId = edu.util.cutPrefixId(/edit/g, strId);
            $(".myModalLabel").html('<i class="fa fa-pencil"></i> Chỉnh sửa');
            edu.util.setOne_BgRow(strId, "tbl_HNCC");
            if (edu.util.checkValue(strId)) {
                me.strCommon_Id = strId;
                me.getDetail_HNCC(strId);
            }
            else {
                edu.system.alert(edu.constant.getting("NOTIFY", "SELECT_F"));
            }
        });
        $("#tbl_HNCC").delegate(".btnDelete", "click", function () {
            var strId = this.id;
    ...
    getList_HNCC: function () {
        ...
        edu.system.makeRequest({
            success: function (data) {
                ...
                if (data.Success) {
                    me.genTable_HNCC(data.Data, data.Pager);
                }
                else {
    ...
    save_HNCC: function () {
        ...
        var obj_save = {
            ...
            'strId': '',
            'strNhomMau_Id': edu.util.getValById("dropNhomMau"),
    ...
    popup_HNCC: function () {
        //show
        edu.util.toggle_overide("zone-bus", "zoneHNC_input");
        //event
        //$('#myModal_QTSK').on('shown.bs.modal', function () {
        //    $('#txtTSBT_TuNgay').val('').trigger('focus').val(value);
        //});
    },
    resetPopup_HNCC: function () {
        var me = this;
        $("#myModalLabel_HuongNghienCuuChinh").html('<i class="fa fa-plus"></i> Thêm hướng nghiên cứu chính');
        me.strCommon_Id = "";
        edu.util.viewValById("txt_HuongNghienCuuChinh", "");
    },
    genTable_HNCC: function (data, iPager) {
        var jsonForm = {
            strTable_Id: "tbl_HNCC",
            aaData: data,
            sort: true,
            colPos: {
                center: [0, 1],
    ...
    editForm_HNCC: function (data) {
        var me = this;
        me.popup_QTSK();
        //view data --Edit
        edu.util.viewValById("dropNhomMau", data.NHOMMAU_ID);
        $("#myModalLabel_QTSK").html('<i class="fa fa-pencil"></i> Chỉnh sửa hướng nghiên cứu chính');
    },
}