...
XepPhongDangKy.prototype = {
    ...
    init: function () {
        ...
        $("#dadangky").delegate('.btnOutNguoiO', 'click', function () {
            var strNguoiO_Id = this.id;
            var strNguoiO_Ten = this.title;
            var confirm = 'Bạn có chắc chắn muốn loại <i class="cl-danger">' + strNguoiO_Ten + '</i> khỏi phòng không?';

            edu.system.confirm(confirm, "q");
            $("#btnYes").click(function (e) {
                me.delete_DangKy(strNguoiO_Id);
            });
        });
        ...
        edu.system.getList_MauImport("zonebtnDK");
    },
    page_load: function () {
        var me = this;
    ...
    loadHopDong: function (data) {
        var me = main_doc.XepPhongDangKy;
    ...
    addHTMLinto_tblSinhVien: function (strNhanSu_Id, strHoTen) {
        ...
        var strNgayVao = localStorage.getItem("KTX_txtNgayVao");
        var strNgayRa = localStorage.getItem("KTX_txtNgayRa");
        if (!edu.util.checkValue(strNgayVao)) strNgayVao = "";
        if (!edu.util.checkValue(strNgayRa)) strNgayRa = "";
        ...
        confirm += '<td>  <input id="txtNgayVao" value="' + strNgayVao + '" class="form-control input-datepicker" placeholder="dd/mm/yyyy"/> </td>';
        confirm += '<td>Ngày ra KTX:</td>';
        confirm += '<td>  <input id="txtNgayRa" value="' + strNgayRa + '" class="form-control input-datepicker" placeholder="dd/mm/yyyy"/> </td>';
        confirm += '</tr>';
        confirm += '</tbody>';
        confirm += '</table>';
        confirm += '<div class="clear"></div>';
        ...
        $("#btnYes").click(function (e) {
            ...
            localStorage.setItem("KTX_txtNgayVao", strNgayVao);
            localStorage.setItem("KTX_txtNgayRa", strNgayRa);
            if (strNgayVao == "") {
                e.stopImmediatePropagation();
                $("#lblConfirmContent2").html('<i class="cl-danger">Vui lòng nhập ngày vào!</i>');
                $("#btnYes").show();
                return;
            }
            //else {
            ...
            //        $("#lblConfirmContent2").html('<i class="cl-danger">Ngày vào phải nhỏ hơn hoặc bằng ngày hiện tại!</i>');
            //        $("#btnYes").show();
            //        return;
            //    }
            //    else if (strNgayRa != "" && edu.util.dateCompare(strNgayRa, edu.util.dateToday()) == -1) {
            //        $("#lblConfirmContent2").html('<i class="cl-danger">Ngày ra phải lớn hơn ngày hiện tại!</i>');
            //        $("#btnYes").show();
            //        return;
            //    }
            //}
            me.save_DangKy(strNhanSu_Id, strNgayVao, strNgayRa);
        });
    },
    ...
    genBox_ToaNhaDangKy: function (data, iPager) {
        ...
        row += '<p>Tổng số phòng còn trống: ' + iTongSoPhongConTrong + '</p>';
    ...
    genList_DangKy: function (json, iPager) {
        ...
        for (var i = 0; i < json.length; i++) {
            ...
            row += '<div class="small-box-footer">';
            row += '<a id="' + data.ID + '" title="' + edu.util.checkEmpty(data.KTX_DOITUONGOKYTUCXA_HODEM) + " " + edu.util.checkEmpty(data.KTX_DOITUONGOKYTUCXA_TEN) + '" class="btn btn btn-danger btnOutNguoiO poiter pull-right" style="margin-right: 6px"><i class="fa fa-trash"></i> Rời phòng</a>';
