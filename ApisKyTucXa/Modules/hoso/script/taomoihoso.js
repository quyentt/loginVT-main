...
KhoiTao.prototype = {
    ...
    init: function () {
       ...
       -------------------------------------------*/
        ...
        --Order:
        -------------------------------------------*/
        $("#tblKhoiTao_NhanSu").delegate('.btnDetailTaiChinh', 'click', function (e) {
            $("#myModalTaiChinh").modal("show");
            var id = this.id;
            var aData = me.dt_NO.find(e => e.ID === id);
            $("#lblSinhVien").html(edu.util.returnEmpty(aData.HODEM) + " " + edu.util.returnEmpty(aData.TEN));
            me.getList_TinhHinhTaiChinh(id);
        });
    },
    page_load: function () {
    ...
    genTable_NO: function (data, iPager) {
        ...
        var jsonForm = {
            ...
            aoColumns: [
                ...
                , {
                    "mRender": function (nRow, aData) {
                        return '<span><a class="btn btn-default btnDetailTaiChinh" id="' + aData.ID + '">Xem TC</span>';
                    }
                }
                , {
                    "mRender": function (nRow, aData) {
    ...
    getList_TinhHinhTaiChinh: function (strKTX_DoiTuongOKTX_Id) {
        var me = this;
        //--Edit
        var obj_list = {
            'action': 'KTX_TinhHinhCongNoKTX/LayDanhSach',
            'type': 'GET',
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    var json = data.Data;
                    me.genTable_TinhHinhTaiChinh(json.rsNo, "tblConNo");
                    me.genTable_TinhHinhTaiChinh(json.rsDu, "tblConDu");

                    me.genTable_TinhHinhTaiChinh_NoiDung(json.rsDaNop, "tblDaNop");
                    me.genTable_TinhHinhTaiChinh_NoiDung(json.rsPhaiNop, "tblPhaiNop");
                    me.genTable_TinhHinhTaiChinh_NoiDung(json.rsMien, "tblMien");
                    me.genTable_TinhHinhTaiChinh_NoiDung(json.rsRut, "tblRut");
                } else {
    ...
    genTable_TinhHinhTaiChinh: function (data, strTable_Id) {
        var me = this;
        var jsonForm = {
        ...
        if (data != undefined && data.length > 0) {
            ...
            $("#" + strTable_Id + " tfoot tr td:eq(2)").attr("style", "text-align: right; font-size: 20px; padding-right: 20px");
        } else {
            $("#" + strTable_Id + " tfoot").html('');
        }
    },
    genTable_TinhHinhTaiChinh_NoiDung: function (data, strTable_Id) {
        var me = this;
        var jsonForm = {
            strTable_Id: strTable_Id,
            aaData: data,
