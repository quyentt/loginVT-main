...
CoiThi.prototype = {
    ...
    init: function () {
        ...
        $(".btnSearchCoiThi_NhanSu").click(function () {
        ...
        $("#btnSaveCoiThi").click(function () {
            me.save_CoiThi();
        });
        $("#btnSearch_CoiThi_NhanSu").click(function () {
            me.getList_NhanSu();
        });
        $("#txtSearch_CoiThi_TuKhoa").keypress(function (e) {
            if (e.which === 13) {
                e.preventDefault();
                me.getList_NhanSu();
            }
        });
        $("#tblCoiThi_NhanSu").delegate(".btnView", "click", function () {
            var strId = this.id;
            ...
            if (edu.util.checkValue(strId)) {
                ...
                me.getList_CoiThi(strId);
            }
            else {
                edu.system.alert(edu.constant.getting("NOTIFY", "SELECT_F"));
            }
        });
        $("#tblCoiThi_NhanSu").delegate('.btnView', 'mouseenter', function (e) {
       ...
       --Order: 
       -------------------------------------------*/
        $("#dropSearch_CoiThi_CCTC").on("select2:select", function () {
       ...
       -------------------------------------------*/
        ...
        $("#tblInput_CoiThi_NhanSu").delegate('.btnRemove', 'click', function () {
            var id = this.id;
    ...
    page_load: function () {
        var me = this;
        edu.util.toggle("box-sub-search");
        edu.util.focus("txtSearch_CoiThi_NhanSu");
        me.toggle_notify();
        edu.system.dateYearToCombo("1993", "dropCoiThi_NamApDung", "Chọn năm áp dụng")
        /*------------------------------------------
    ...
    rewrite: function () {
        ...
        me.strCoiThi_Id = "";
        //
        var arrId = ["txtCoiThi_NgayBatDau", "txtCoiThi_NgayKetThuc", "txtCoiThi_SoNgayNghi", "txtCoiThi_ThamNien", "dropCoiThi_NamApDung"];
        edu.util.resetValByArrId(arrId);
        //
        $("#ckbCoiThi_All").prop('checked', false);
        $("#tblInput_CoiThi_NhanSu tbody").html("");
        me.arrNhanSu_Id = [];
    },
    toggle_detail: function () {
        edu.util.toggle_overide("zoneCoiThi", "zone_detail_CoiThi");
    },
    toggle_input: function () {
        edu.util.toggle_overide("zoneCoiThi", "zone_input_CoiThi");
    },
    toggle_notify: function () {
        edu.util.toggle_overide("zoneCoiThi", "zone_notify_CoiThi");
    },
    /*------------------------------------------
    ...
    getList_NhanSu: function () {
        ...
        var strCoCauToChuc = edu.util.getValById("dropSearch_CoiThi_BoMon");
        if (!edu.util.checkValue(strCoCauToChuc)) {
            strCoCauToChuc = edu.util.getValById("dropSearch_CoiThi_CCTC");
        }

        var obj = {
            strTuKhoa: edu.util.getValById("txtSearch_CoiThi_TuKhoa"),
    ...
    genTable_NhanSu: function (data, iPager) {
        var me = main_doc.CoiThi;
        me.dtNhanSu = data;
        edu.util.viewHTMLById("lblCoiThi_NhanSu_Tong", iPager);

        var jsonForm = {
            strTable_Id: "tblCoiThi_NhanSu",
            aaData: data,
            bPaginate: {
                strFuntionName: "main_doc.CoiThi.getList_NhanSu()",
                iDataRow: iPager
            },
            bHiddenHeader: true,
            bHiddenOrder: true,
    ...
    genCombo_CCTC_Parents: function (data) {
        var obj = {
            ...
            renderPlace: ["dropSearch_CoiThi_CCTC"],
            type: "",
    ...
    genCombo_CCTC_Childs: function (data) {
        var obj = {
            ...
            renderPlace: ["dropSearch_CoiThi_BoMon"],
            type: "",
            title: "Bộ môn"
        };
        edu.system.loadToCombo_data(obj);
    ...
    getList_CoiThi: function () {
        ...
        //--Edit
        var obj_list = {
            'action': 'TKGG_HoatDong_CoiThi/LayDanhSach',
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    me.dtCoiThi = dtResult;
                    me.genTable_CoiThi(dtResult, iPager);
                }
                else {
                    edu.system.alert("TKGG_HoatDong_CoiThi/LayDanhSach: " + data.Message, "w");
                }
                
            },
            error: function (er) {
                edu.system.alert("TKGG_HoatDong_CoiThi/LayDanhSach (er): " + JSON.stringify(er), "w");
    ...
    save_CoiThi: function () {
        var me = this;
        var strNhanSu_Ids = "";

        //check all NhanSu or list of NhanSu
        if ($("#ckbCoiThi_All").is(':checked')) {
            strNhanSu_Ids = "ALL";
        }
        else {
        ...
        //--Edit
        var obj_save = {
            'action': 'TKGG_HoatDong_CoiThi/ThemMoi',
            

            'strId': "",
            'strNhanSu_HoSoCanBo_Id': strNhanSu_Ids,
            'strNgayBatDau': edu.util.getValById("txtCoiThi_NgayBatDau"),
            'strNgayKetThuc': edu.util.getValById("txtCoiThi_NgayKetThuc"),
            'strNamApDung': edu.util.getValById("dropCoiThi_NamApDung"),
            'dSoNgayDuocNghi': edu.util.getValById("txtCoiThi_SoNgayNghi"),
            'dSoNgayNghiThamNien': edu.util.getValById("txtCoiThi_ThamNien"),
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    edu.system.alert("TKGG_HoatDong_CoiThi/ThemMoi: " + data.Message);
                }
                
            },
            error: function (er) {
                edu.system.alert("TKGG_HoatDong_CoiThi/ThemMoi (er): " + JSON.stringify(er), "w");
    ...
    update_CoiThi: function () {
        var me = this;
        //--Edit
        var obj_save = {
            'action': 'TKGG_HoatDong_CoiThi/CapNhat',
            

            'strId': me.strCoiThi_Id,
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    edu.system.alert("TKGG_HoatDong_CoiThi/CapNhat: " + data.Message);
                }
                
            },
            error: function (er) {
                edu.system.alert("TKGG_HoatDong_CoiThi/CapNhat (er): " + JSON.stringify(er), "w");
    ...
    getDetail_CoiThi: function (strId) {
        var me = this;
        edu.util.objGetDataInData(strId, me.dtCoiThi, "ID", me.viewDetail_CoiThi);
    },
    delete_CoiThi: function (strId) {
        var me = this;
        //--Edit
        var obj = {};
        var obj_delete = {
            'action': 'TKGG_HoatDong_CoiThi/Xoa',
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    obj = {
                        content: "TKGG_HoatDong_CoiThi/Xoa: " + data.Message,
                        code: ""
                    };
                    edu.system.afterComfirm(obj);
                }
            ...
            error: function (er) {
                var obj = {
                    content: "TKGG_HoatDong_CoiThi/Xoa (er): " + JSON.stringify(er),
                    code: "w"
                };
                edu.system.afterComfirm(obj);
    ...
    genTable_CoiThi: function (data, iPager) {
        ...
        var jsonForm = {
            ...
            aaData: data,
            bPaginate: {
                strFuntionName: "main_doc.CoiThi.getList_CoiThi()",
                iDataRow: iPager
            },
            bHiddenHeader: true,
            colPos: {
            ...
            aoColumns: [
                ...
                , {
                    "mDataProp": "CANBOCOITHI_TEN"
                }
                , {
                    "mDataProp": "MONTHI_TEN"
                }
                , {
    ...
    getDetail_NhanSu: function (strId) {
        var me = this;
        var dt = [];
        $("#lblCoiThi_NhanSu").html("");
        var $hoten = "#lbl" + strId;
        var valHoTen = $($hoten).html();
        $("#lblCoiThi_NhanSu").html(valHoTen);
    },
    viewDetail_CoiThi: function (data) {
        var me = main_doc.CoiThi;
    ...
    addHTMLinto_tblNhanSu: function (strNhanSu_Id) {
        ...
        $("#tblInput_CoiThi_NhanSu tbody").append(html);
    },
    removeHTMLoff_tblNhanSu: function (strNhanSu_Id) {
        ...
        if (me.arrNhanSu_Id.length === 0) {
            $("#tblInput_CoiThi_NhanSu tbody").html("");
            $("#tblInput_CoiThi_NhanSu tbody").html('<tr><td colspan="6" class="td-center">Không tìm thấy dữ liệu!</td></tr>');
        }
    }
};