...
HeSoPhamVi.prototype = {
    ...
    init: function () {
        ...
        me.getList_HeSoPhamVi();
        me.getList_ThoiGianDaoTao();
        edu.system.loadToCombo_DanhMucDuLieu("KLGD.PHANLOAIXACNHAN", "dropPhamViApDung");
        $("#tblHeSoPhamVi").delegate(".btnEdit", "click", function () {
            var strId = this.id;
            var data = me.dtHeSoPhamVi.find(e => e.ID == strId);
            me["strHeSoPhamVi_Id"] = data.ID;
            ...
            $("#myModal_HeSoPhamVi").modal("show");
            $("#lblPhamVi").html(data.PHAMVIAPDUNG_TEN)
        });
        $("#btnAdd_HeSoPhamVi").click(function () {
            var data = {};
            me["strHeSoPhamVi_Id"] = data.ID;
            edu.util.viewValById("dropPhanLoai", data.LOAIBANG_ID);
            edu.util.viewValById("dropPhamVi", data.PHAMVIAPDUNG_ID);
        ...
        $("#btnSave_HeSoPhamVi").click(function () {
            me.save_HeSoPhamVi();
        });
        $("#btnDelete_HeSoPhamVi").click(function () {
            var arrChecked_Id = edu.util.getArrCheckedIds("tblHeSoPhamVi", "checkX");
            ...
            $("#btnYes").click(function (e) {
                ...
                for (var i = 0; i < arrChecked_Id.length; i++) {
                    me.delete_HeSoPhamVi(arrChecked_Id[i]);
                }
            });
        });
    ...
    --ULR:  Modules
    -------------------------------------------*/
    save_HeSoPhamVi: function () {
        var me = this;
        var obj_notify = {};
        //--Edit
        var obj_save = {
            'action': 'NS_KLGD_ThongTin_MH/FSkkLB4KDQYFHgkkEi4eESkgLBcoADEFNC8m',
            'func': 'PKG_KLGV_V2_THONGTIN.Them_KLGD_HeSo_PhamViApDung',
            'iM': edu.system.iM,
            'strId': me.strHeSoPhamVi_Id,
        ...
        if (obj_save.strId) {
            obj_save.action = 'NS_KLGD_ThongTin_MH/EjQgHgoNBgUeCSQSLh4RKSAsFygAMQU0LyYP';
            obj_save.func = 'PKG_KLGV_V2_THONGTIN.Sua_KLGD_HeSo_PhamViApDung'
        }
        //default
    ...
    getList_HeSoPhamVi: function (strDanhSach_Id) {
        var me = this;
        //--Edit
        var obj_save = {
            'action': 'NS_KLGD_ThongTin_MH/DSA4BRIKDQYFHgkkEi4eESkgLBcoADEFNC8m',
            'func': 'PKG_KLGV_V2_THONGTIN.LayDSKLGD_HeSo_PhamViApDung',
            'iM': edu.system.iM,
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    me.dtHeSoPhamVi = dtReRult;
                    me.genTable_HeSoPhamVi(dtReRult, data.Pager);
                }
                else {
    ...
    delete_HeSoPhamVi: function (Ids) {
        var me = this;
        //--Edit
        var obj_save = {
            'action': 'NS_KLGD_ThongTin_MH/GS4gHgoNBgUeCSQSLh4RKSAsFygAMQU0LyYP',
            'func': 'PKG_KLGV_V2_THONGTIN.Xoa_KLGD_HeSo_PhamViApDung',
            'iM': edu.system.iM,
            'strId': Ids,
            'strNguoiThucHien_Id': edu.system.userId,
        };
    ...
    --ULR:  Modules
    -------------------------------------------*/
    genTable_HeSoPhamVi: function (data, iPager) {
        $("#lblHeSoPhamVi_Tong").html(iPager);
        var jsonForm = {
            strTable_Id: "tblHeSoPhamVi",
            aaData: data,
            //bPaginate: {
            //    strFuntionName: "main_doc.HeSoPhamVi.getList_HeSoPhamVi()",
            //    iDataRow: iPager
            //},
            colPos: {
                center: [0],
