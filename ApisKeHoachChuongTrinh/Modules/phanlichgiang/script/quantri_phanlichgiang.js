...
--Note: 
----------------------------------------------*/
function PhanLichGiang() { };
PhanLichGiang.prototype = {
    strLichHoc_Id: '',
    dtLichHoc: [],
    strPhanGiang_Id: '',
    dtPhanGiang: [],
    ...
    init: function () {
        ...
        $("#btnPhanGiangLich").click(function () {
            ...
            //    arrChecked_Id = arrChecked_Id.slice(0, 100);
            //    edu.system.alert("Vượt quá 100 đối tượng, tự động lấy 100 đối tượng đầu?");
            //}
        ...
        $("#btnSave_LichPhanGiang").click(function () {
            me.iDemBreak = 0;
            me.iDemPhanGiang = $("#tblLichPhanGiang tbody tr");
        ...
        //$("#tblKeThuaLopHocPhan").delegate(".checkOne", "click", function () {
        //    edu.util.checkedOne_BgRow(this, { table_id: "tblKeThuaLopHocPhan", regexp: /checkX/g, });
        //});
        ...
        edu.system.getList_MauImport("zonebtnBaoCao_PLG", function (addKeyValue) {
            ...
            addKeyValue("strLopHocPhan_Id", main_doc.PhanLichGiang.strLopHocPhan_Id);
            addKeyValue("strThoiGianDaoTao_Id", edu.util.getValById("dropSearch_ThoiGianDaoTao"));
            addKeyValue("strCoCauToChuc_Id", edu.util.getValById("dropSearch_CoCauToChuc"));
            ...
            addKeyValue("strCanBo_Id", edu.util.getValById("dropSearch_ThanhVien"));
        ...
        $("#btnImport_PG").click(function () {
            me.delete_PhanGiang_Import("");
            edu.system.showImportChung("Lịch giảng", "IMPORTWITHPROC_LICHGIANG");
        });
        $("#btnTinhToan").click(function () {
            edu.system.confirm("Bạn có chắc chắn muốn tổng hợp giờ giảng và quy đổi không");
            $("#btnYes").click(function (e) {
                me.save_TinhToanTongHop();
            });
        });
    },
    page_load: function () {
    ...
    toggle_editBaiHoc: function () {
        ...
        //edu.util.viewValById("dropBaiHoc_HocPhan", '');
        //edu.util.viewValById("dropBaiHoc_ChuongTrinh", '');
    ...
    save_PhanGiang_LichHoc: function (strId, strTKB_LichGiang_HocPhan_Id) {
        ...
        var strNgayHoc = edu.util.getValById('txtPhanGiang_NgayHoc' + strId);
        ...
        if (strId.length == 30) strId = "";
            ...
            var strCheck = $("#tblLichPhanGiang #" + strId).attr("name");
            var strTemp = strNgayHoc + dSoTiet + dTietBatDau + dTietKeThuc + dSiSo + strDaoTao_BaiHoc_Id + strIdHinhThucHoc + strNhanSu_HoSoCanBo_Id + strTKB_PhanLoaiDiaDiem_Id + strMoTa;
            if (strCheck == strTemp) {
                me.iDemBreak++;
                if ((me.iDemPhanGiang - me.iDemBreak) == 0) edu.system.alert("Không có thay đổi để lưu");
                return;
            }
        }
        var obj_notify;
    ...
    genHTML_PhanGiang_LichHoc: function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            var strCheck = "" + edu.util.returnEmpty(data[i].NGAYHOC) + edu.util.returnEmpty(data[i].LICHHOC_SOTIET) + edu.util.returnEmpty(data[i].LICHHOC_TIETBATDAU) + edu.util.returnEmpty(data[i].LICHHOC_TIETKETTHUC) + edu.util.returnEmpty(data[i].SOSINHVIEN)
                + edu.util.returnEmpty(data[i].DAOTAO_BAIHOC_ID) + edu.util.returnEmpty(data[i].IDHINHTHUCHOC) + edu.util.returnEmpty(data[i].NHANSU_HOSOCANBO_ID) + edu.util.returnEmpty(data[i].TKB_PHANLOAIDIADIEM_ID) + edu.util.returnEmpty(data[i].MOTA);

            var row = '';
            row += '<tr id="' + strPhanGiang_LichHoc_Id + '" ' + strStyle + ' name="'+ strCheck +'">';
            row += '<td style="text-align: center"><label id="txtStt' + strPhanGiang_LichHoc_Id + '">' + (i + 1) + '</label></td>';
            row += '<td style="text-align: center"><input type="text" style="min-width: 100px" id="txtPhanGiang_NgayHoc' + strPhanGiang_LichHoc_Id + '" value="' + edu.util.returnEmpty(data[i].NGAYHOC) + '" class="form-control"/></td>';//Phân giảng - Số tiết
            ...
            row += '<td style="text-align: center"><input type="text" style="min-width: 48px" id="txtPhanGiang_SiSo' + strPhanGiang_LichHoc_Id + '" value="' + edu.util.returnEmpty(data[i].SOSINHVIEN) + '" class="form-control"/></td>';//Phân giảng - Số tiết
            row += '<td style="text-align: center"><input type="text" style="min-width: 48px" id="txtPhanGiang_SoTiet' + strPhanGiang_LichHoc_Id + '" value="' + edu.util.returnEmpty(data[i].SOTIET) + '" class="form-control"/></td>';//Phân giảng - Số tiết
            row += '<td style="text-align: center"><input type="text" style="min-width: 48px" id="txtPhanGiang_TietBatDau' + strPhanGiang_LichHoc_Id + '" value="' + edu.util.returnEmpty(data[i].TIETBATDAU) + '" class="form-control"/></td>';//Phân giảng - Số tiết
            row += '<td style="text-align: center" ><input type="text" style="min-width: 48px" id="txtPhanGiang_TietKetThuc' + strPhanGiang_LichHoc_Id + '" value="' + edu.util.returnEmpty(data[i].TIETKETTHUC) + '" class="form-control"/></td>';//Phân giảng - Số tiết
            ...
            //row += '<td><select id="dropDiaDiem_KhuVuc' + strPhanGiang_LichHoc_Id + '" class="select-opt"><option value=""> --- Chọn khu vực--</option ></select ></td>';
            //row += '<td><select id="dropDiaDiem_ToaNha' + strPhanGiang_LichHoc_Id + '" class="select-opt"><option value=""> --- Chọn tòa nhà--</option ></select ></td>';
            //row += '<td><select id="dropDiaDiem_GiangDuong' + strPhanGiang_LichHoc_Id + '" class="select-opt"><option value=""> --- Chọn giảng đường--</option ></select ></td>';
            ...
            row += '<td style="text-align: center"><a title="Xóa" class="deletePhanGiang_LichHoc" id="' + strPhanGiang_LichHoc_Id + '" href="javascript:void(0)" style="color: red">Xóa</a></td>';
            row += '</tr>';
        ...
        for (var i = 0; i < me.dtHoatDong.length; i++) {
            $("#zoneSoTietLichGiang").append(me.dtHoatDong[i].TENHINHTHUCHOC + " : <b>" + me.dtHoatDong[i].SOLUONG + "; </b>");
        }
        function moveScroll() {
    ...
    genHTML_PhanGiang_LichHoc_New: function (strPhanGiang_LichHoc_Id) {
        ...
        row += '<td><input type="text" style="min-width: 100px" id="txtPhanGiang_NgayHoc' + strPhanGiang_LichHoc_Id + '" class="form-control"/></td>';
        ...
        row += '<td><input type="text" style="min-width: 48px" id="txtPhanGiang_SiSo' + strPhanGiang_LichHoc_Id + '" class="form-control"/></td>';
        row += '<td><input type="text" style="min-width: 48px" id="txtPhanGiang_SoTiet' + strPhanGiang_LichHoc_Id + '" class="form-control"/></td>';
        row += '<td><input type="text" style="min-width: 48px" id="txtPhanGiang_TietBatDau' + strPhanGiang_LichHoc_Id + '" class="form-control"/></td>';
        row += '<td><input type="text" style="min-width: 48px" id="txtPhanGiang_TietKetThuc' + strPhanGiang_LichHoc_Id + '" class="form-control"/></td>';
        row += '<td><select id="dropPhanGiang_BaiHoc' + strPhanGiang_LichHoc_Id + '" class="select-opt"><option value=""> --- Chọn bài học--</option ></select ></td>';
        ...
        //me.genCombo_PhanGiang_ToaNha("dropDiaDiem_ToaNha" + strPhanGiang_LichHoc_Id, "");
        //me.genCombo_PhanGiang_GiangDuong("dropDiaDiem_GiangDuong" + strPhanGiang_LichHoc_Id, "");
    },
    /*------------------------------------------
    --Discription: [4]  ACESS DB ==> thoi gian dao tao
    --Author: duyentt
    ...
    genTable_BaiHoc: function (data) {
        //$("#lblHP_BaiHoc_Tong").html(iPager);
        var jsonForm = {
            strTable_Id: "tblBaiHoc",
            aaData: data,
            //bPaginate: {
            //    strFuntionName: "main_doc.PhanLichGiang.getList_BaiHoc()",
            //    iDataRow: iPager
            //},
            sort: true,
            colPos: {
    ...
    loadToCombo_DiaDiem: function (data) {
        main_doc.PhanLichGiang.dtDiaDiem = data;
    },
    ...
    --Discription: [1] AcessDB GetList_DeTai
    -------------------------------------------*/
    delete_PhanGiang_Import: function (strId) {
        var me = this;
        //--Edit
        var obj_delete = {
            'action': 'KHCT_LichGiang_Import/Xoa',
    ...
    --Discription: lop hoc
    ----------------------------------------------*/
    save_TinhToanTongHop: function (strId) {
        var me = this;
        //--Edit
        var obj_save = {
            'action': 'KHCT_TinhToan/TongHopGioGiangVaQuyDoi',
            'type': 'GET',
            'strDaoTao_CoCauToChuc_Id': edu.util.getValById('dropSearch_DonViThanhVien'),
            'strDaoTao_ThoiGianDaoTao_Id': edu.util.getValById('dropSearch_ThoiGianDaoTao'),
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    edu.system.alert("Thực hiện hoàn tất. Hãy kiểm tra lại!", "w");
                }
                else {
                    edu.system.alert(obj_save.action + ": " + data.Message);
                }
