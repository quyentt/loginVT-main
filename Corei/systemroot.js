...
systemroot.prototype = {
    ...
    startApp: function () {
        ...
        $(document).keydown(function (event) {
            if (event.ctrlKey) {
                switch (event.which) {
                    ...
                    case 188: me.showBaoCao(); break;
                    case 89: me.showAllId(); break;
                }
            }
        });

        $(document.body).on('show.bs.modal shown.bs.modal hide.bs.modal hidden.bs.modal', function () {
            $('body').css('padding-right', '0');
        });

        $("#tblApp").delegate('.ungdung', 'click', function (e) {
            var strId = this.id;
            var objUngDung = me.dtUngDung.find(e => e.ID === strId);
            me.setUngDung(objUngDung);
            $("#btnSileApp").trigger("click")
        });
        ...
        //async Upload
        //$(me.ctPlacehoder).append('<script src="' + me.rootPathUpload + '/Core/uploadfile.js?v=1.0.0.11"></script>');
        ...
        if (me.objApi.urlNode) {
            me["idemNode"] = 0;
            $(me.ctPlacehoder).append('<script async src="' + me.objApi.urlNode + '/socket.io/socket.io.js"></script>');
            setTimeout(function () { me.nodeChat(me.objApi.urlNode); }, 1000);
        }
        var strModal = '';
        ...
        $(document).delegate(".btnEdit_MauBaoCao", "click", function (e) {
            ...
            if ($("#modalMauBaoCao").length == 0) {
                ...
                strModal += '<input type="text" id="txtTenHienThiMauBaoCao" class="form-control" />';
                strModal += '</div>';
                ...
                strModal += '</div>';
                strModal += '<div class="col-sm-9">';
                strModal += '<div id="importToCheck"></div>';
                strModal += '</div>';
                strModal += '<div class="clear"></div>';
                strModal += '</div>';
                strModal += '<div class="row">';
    ...
    versionPageJS: function () {
        ...
        jQuery.ajax({
            ...
            if (responseText.length != me.urlPageLength) {
                console.log("versionPageJS: " + responseText.length + ":" + me.urlPageLength)
    ...
    */
    makeRequest: function (params, inc_user, inc_code, inc_lang, container, urlService, block) {
        ...
        $.ajax({
            ...
            crossDomain: true,
            url: strUrl,
            headers: { 'Authorization': strtokenJWT },
            data: dataPost,
            cache: false,
        ...
        if (op.type === "POST") me.makeLog("POST", JSON.stringify(dataPost), strUrl);
    },
    makeLog: function (strHanhDong, strMoTa, strUrl) {
        var me = this;
        return;
        switch (strHanhDong) {
            case "VAOCHUCNANG": {
        ...
        if (strHanhDong == "POST") {
            /* log tham so API 
            console.log(strUrl)
            strMoTa = JSON.parse(strMoTa);
            delete strMoTa["action"];
            delete strMoTa["strChucNang_Id"];
            var z = "";
            for (var x in strMoTa) {
                 z+="\n"+x;
            }
            console.log(z);
            strMoTa = JSON.stringify(strMoTa);
            strMoTa = strMoTa.replace(/,/g, ',\n\t');
            strMoTa = strMoTa.replace(/{/g, '{\n\t');
            strMoTa = strMoTa.replace(/}/g, '\n}');
            console.log(strMoTa)
            */
    ...
    */
        ...
        function uploadFile(arrFile) {
            for (var i = 0; i < arrFile.length; i++) {
                ...
                $(document).delegate(".btnDeleteFileUp" + elementId, "click", function (e) {
                    ...
                    while (pointNode != null && pointNode.nodeName != "UL") {
    ...
    saveFiles: function (strZone, strDuLieu_Id, strApi, callback) {
        ...
        if (document.getElementById(strZone) == undefined) {
            me.alert("Không tìm thấy: " + strZone, "w");
            return;
        }
    ...
    */
    ...
    pagInfoRender: function (strTable_Id) {
        ...
        $('#dropPageSizechange' + strTable_Id).select2({
            tags: true,
            createTag: params => /^\d+$/.test(params.term) ? { id: params.term, text: params.term } : null,
        });

        //End zone page header
        // Cho phép gõ số và nhấn Enter để load ngay
        let ddlId = '#dropPageSizechange' + strTable_Id;
        let $ddl = $(ddlId);

        $ddl.on('select2:open', function () {
            const $search = $('.select2-container--open .select2-search__field');

            // Gỡ binding cũ để tránh nhân đôi
            $search.off('keydown.pagesize').on('keydown.pagesize', function (e) {
                if (e.key === 'Enter') {
                    e.preventDefault();
                    e.stopImmediatePropagation(); // Chặn Select2 chọn option mặc định

                    const val = $(this).val().trim();
                    if (!/^\d+$/.test(val)) return; // chỉ nhận số

                    // Thêm option nếu chưa có
                    if ($ddl.find("option[value='" + val + "']").length === 0) {
                    ...
                    if (typeof dropPageSizeChange === 'function') {
                        dropPageSizeChange($ddl[0]);
                    }

                    // Đóng dropdown sau 1 tick
                    setTimeout(() => $ddl.select2('close'), 30);
                }
            });
        });
        ...
        // Cho phép nhập số rồi Enter trong dropdown phân trang
    ...
    insertFilterToTable: function (strTable_Id, strFuntionName, iPager) {
        var me = this;
        var row_filter = '';

        row_filter += '<div class="input-group pull-right">';
        row_filter += '<input id="' + strTable_Id + '_input" type="text" name="q" class="form-control" style="width: 250px; float: right" placeholder="Tìm theo từ khóa">';
        if (iPager == 1) {
            row_filter += '<span>';
            row_filter += '<a id="lblRow' + strTable_Id + '"></a>';
            row_filter += '</span>';
        } else {
        ...
        if (iPager == 1) {
            $("#" + strTable_Id + "_input").on("keyup", function () {
                ...
                let rowCount = $(this).val() ? $('#' + strTable_Id + ' tbody tr:visible').length : "";
                $("#lblRow" + strTable_Id).html(rowCount);
            });
        } else {
   ...
   */
    loadToTable_data: function (obj) {
        ...
        if ($("#" + strTableId + " tbody").length === 0) {
            console.log("Không tìm thấy: " + strTableId);
            return;
        }
        else {
            console.log("loadToTable_data: " + obj.strTable_Id);
        }
        mainTable();
        //
        function mainTable() {
        ...
        function addPagination() {
            if (obj.bPaginate != undefined && obj.bPaginate != false) {
                ...
                if (!edu.util.checkValue($("#light-pagination" + strTableId).html())) {
                    me.pagInfoRender(strTableId);
                    if (obj.bPaginate.bFilter == true) me.insertFilterToTable(strTableId, obj.bPaginate.strFuntionName, obj.bPaginate.iDataRow);
   ...
   */
        ...
        function genHTML_HangDoi(data, iPager) {
            ...
            if (data.length > 0) {
                ...
                if (count_HangDoi == data.length) {
                    ...
                    var row = '<div class="col-sm-3 item-search" style="min-width: 220px">';
                    row += '<select id="dropSearch_SoLuong_' + objHangDoi.strName + '" class="select-opt" style="width:100% !important">';
                    row += '<option id="" value="10">10 luồng cùng chạy</option>';
                    row += '<option id="" value="2">Chạy tuần tự</option>';
                    row += '<option id="" value="30">30 luồng cùng chạy</option>';
                    row += '<option id="" value="50">50 luồng cùng chạy</option>';
                    row += '<option id="" value="100">100 luồng cùng chạy</option>';
                    row += '</select>';
                    row += '</div>';
                    $(place).prepend(row);
                    $('#dropSearch_SoLuong_' + objHangDoi.strName).on('change', function () {
                        me.iGioiHanLuong = $('#dropSearch_SoLuong_' + objHangDoi.strName).val();
                    });
                }
            }
        }
            ...
            me.makeRequest({
                success: function (data) {
                    if (data.Success) {
                        ...
                        if (me.iGioiHanLuong == 2) {
                            me.genHTML_Progress('probar_' + objHangDoi.strName + strHangDoi_Id, Math.ceil(iTongDuLieu / 20));
                            //me.iGioiHanLuong = 1;
                            var arrId = [];
                            for (var i = 0; i < iTongDuLieu; i++) {
                                arrId.push(data.Data[i].ID);
                                if (arrId.length == 20) {
                                    save_XuLyNhiemVu(arrId, i);
                                    arrId = [];
                                }
                            }
                        } else {
                            ...
                            //me.iGioiHanLuong = 1;
                            for (var i = 0; i < iTongDuLieu; i++) {
                                //if (data.Data[i].TINHTRANG == 0) continue;
    ...
    */
    getList_LopQuanLy: function (obj, resolve, reject, callback) {
        ...
        var strDaoTao_CoSoDaoTao_Id = edu.util.returnEmpty(obj.strCoSoDaoTao_Id);
        var strDaoTao_KhoaDaoTao_Id = edu.util.returnEmpty(obj.strKhoaDaoTao_Id);
        var strDaoTao_KhoaQuanLy_Id = edu.util.returnEmpty(obj.strDaoTao_KhoaQuanLy_Id);
    ...
    */
    getList_NamNhapHoc: function (obj, resolve, reject, callback) {
        ...
        var obj_save = {
            'action': 'KHCT_ThongTin_MH/DSA4BRIPICwPKSAxCS4i',
            'func': 'pkg_kehoach_thongtin.LayDSNamNhapHoc',
            'iM': edu.system.iM,
            'strNguoiThucHien_Id': me.userId
        };
    ...
    */
    getList_NguoiDung: function (obj, resolve, reject, callback) {
        ...
        //    pageSize: 100000
        //};
        //edu.system.getList_NguoiDung(obj, resolve, reject, callback);

        var iTrangThai = 1;//get user active
        var strNguoiThucHien_Id = edu.util.returnEmpty(obj.strNguoiThucHien_Id);
        var strTuKhoa = edu.util.returnEmpty(obj.strTuKhoa);
        var strPhanLoaiDoiTuong = edu.util.returnEmpty(obj.strPhanLoaiDoiTuong);
        var strChung_DonVi_Id = edu.util.returnEmpty(obj.strChung_DonVi_Id);
        var strVaiTro_Id = edu.util.returnEmpty(obj.strVaiTro_Id);
        var strCapXuLy_Id = edu.util.returnEmpty(obj.strCapXuLy_Id);
        var strTinhThanh_Id = edu.util.returnEmpty(obj.strTinhThanh_Id);
        var pageIndex = edu.util.returnZero(obj.pageIndex);
        var pageSize = edu.util.returnZero(obj.pageSize);
        ...
        var obj_list = {
            ...
            'pageIndex': pageIndex,
            'pageSize': pageSize,
            'strPhanLoaiDoiTuong': strPhanLoaiDoiTuong,
            'strChung_DonVi_Id': strChung_DonVi_Id,
            'strVaiTro_Id': strVaiTro_Id,
            'strCapXuLy_Id': strCapXuLy_Id,
            'strTinhThanh_Id': strTinhThanh_Id,
        };
        ...
        me.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    me.alert("CM_NguoiDung.LayDanhSach: " + data.Message, "w");
                    me.endLoading();
                }
            },
            error: function (er) {
    ...
    */
    ...
    checkChucNang: function () {
        ...
        if (!me.appId) {
            ...
            if (!strChucNang_Id) location.href = "./index.aspx";
                    ...
                    if (data.Success) {
                        ...
                        if (data.length === 1) {
                            ...
                            data.forEach((e, index) => {
                                let strAnhIcon = e.TENANH ? e.TENANH : 'fa fa-cart-plus';
                                html += '<tr class="dashboad-item ungdung" id="' + e.ID + '">';
                                html += '<td class="td-middle fa-app">';
                                html += '<a title="' + e.TENVAITRO + '"><span class="fa-stack"><i class="fa fa-circle fa-stack-2x user-icon-bg"></i><i class="' + strAnhIcon + ' fa-stack-1x text-primary user-icon"></i></span></a>';
                                html += '</td>';
                                html += '<td class="td-middle">';
                                html += '<a title="' + e.TENVAITRO + '" >' + e.TENVAITRO + '</a>';
                                html += '</td>';
                                html += '</tr>';
                                //html += '<div class="dashboad-item ungdung" id="' + e.ID + '">';
                                //html += '<a href="#" class="dashboad-item-box " style="border-color:' + arrMau[index % 8] + '">';
                                //html += '<img src="' + e.TENANH + '" alt="' + e.TENANH + '">';
                            ...
                            if (html) $("#tblApp tbody").html(html);
                            if (me.appId) {
                                var objUngDung = me.dtUngDung.find(e => e.ID === me.appId);
                                $("#lbLogo_mini").html(objUngDung.TENVAITRO);
                                $("#lbLogo_large").html(objUngDung.TENVAITRO);
    ...
    */
    ...
    -- date: 02/06/2018
    */
    loadToCombo_DanhMucDuLieu: function (strCode, zone_id, type, callback, title, strTenCotSapXep) {
        //--------------------------Usage------------------------------------
    ...
    confirm: function (content, code) {
        ...
        function genBox_Alert() {
            if (!me.flag_alert) {
                ...
                confirm += '<button type="button" class="btn btn-primary" id="btnYes"><i class="fa fa-check-circle"></i> ' + edu.constant.getting("BUTTON", "YES") + '</button>';
                confirm += '<button type="button" class="btn btn-default" data-dismiss="modal"><i class="fa fa-times-circle"></i> ' + edu.constant.getting("BUTTON", "CLOSE") + '</button>';
                confirm += '</div>';
                confirm += '</div>';
    ...
    */
    pickerdate: function (strClassName) {
        ...
        var x = document.getElementsByClassName(strClassName);
        var check = window.innerWidth > 800; // không hiển thị với màn hình nhỏ
        for (var i = 0; i < x.length; i++) {
        ...
        //Update bắt buộc ngày phải đúng định dạng
        //$(document).delegate('.input-datepicker', 'keyup', function (e) {
        //    var point = this;
        //    if (e.which == 13) {
        //        point.value = me.strdate;
        //    }
        //});
        //$(document).delegate('.input-datepicker', 'keypress', function (e) {
        //    var point = this;
        //    if (e.which != 13) {
        //        console.log(point.value);
        //        me.strdate = point.value;
        //    }
        //});
    ...
    */
    ...
    genHTML_MenuVertical: function (data) {
        ...
        if (strChucNang_Id != "" || strChucNang_Id != undefined) {
            ...
            if (x.ID != undefined) {
                $("#chucnang" + strChucNang_Id).find("a").trigger("click");
                if (x.CHUCNANGCHA_ID != null && x.CHUCNANGCHA_ID != "") {
                    var pointChucNang = document.getElementById("chucnang" + x.CHUCNANGCHA_ID);
                    pointChucNang.classList.add("menu-open");
                    $(pointChucNang).find("ul").attr("style", "display: block");
                    var y = edu.util.objGetOneDataInData(x.CHUCNANGCHA_ID, data, "ID");
                    if (y.ID != undefined) {
                        var pointChucNang = document.getElementById("chucnang" + x.CHUCNANGCHA_ID);
                        pointChucNang.classList.add("menu-open");
                        $(pointChucNang).find("ul").attr("style", "display: block");
    ...
    genHTML_MenuVertical_Recusive: function (data, parent_id, nodeMenuVertical) {
        ...
        for (var i = 0; i < data.length; i++) {
            if (data[i].CHUCNANGCHA_ID == parent_id) {
                ...
                if (!edu.util.arrCheckExist(me.arrMenu_HasChild, parent_id)) {
    ...
    genPath_ChucNang: function () {
        ...
        if (edu.util.checkValue(data[0].DUONGDANHUONGDANSUDUNG)) {
            if (data[0].DUONGDANHUONGDANSUDUNG.indexOf('http') == 0) {
            ...
            if (data[0].DUONGDANHUONGDANSUDUNG.indexOf("$") != -1) {
        ...
        if (typeof (Storage) !== "undefined") {
            sessionStorage.setItem("strChucNang_Id", data[0].ID);
        }
        ...
        function getNameChucNang(json) {
            var row = '<span class="sub-path opacity-5"> / ' + json.TENCHUCNANG + '</span>';
    ...
    hiddenElement: function (strHidden) {
        console.log("hiddenElement: " + strHidden);
    ...
    genHTML_Progress: function (strDiv_Id, iToTal, iBatDau) {
        ...
        row += '</div>';
        row += '<div id="zoneShowLog">' + iBatDau + '/' + iToTal + ' (0%)</div>';
        row += '</div>';
        $("#" + strDiv_Id).html(row);
    },
    ...
    getList_MauImport: function (strZoneButton, callback) {
        ...
        $("#" + strZoneButton + "_DatLich").delegate(".btnBaoCao_LHD", "click", function (e) {
            e.preventDefault();
            me.report($(this).attr("name"), $(this).attr("duongdan"), callback, undefined, undefined, true);
        });
        //
        ...
        me.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    me.alert(obj_list.action + ": " + data.Message, "w");
                }
            },
            error: function (er) {
                me.alert(obj_list.action + " (er): " + JSON.stringify(er), "w");
            },
            type: "GET",
            action: obj_list.action,
    ...
    cbGenCombo_MauImport: function (data, strZoneButton) {
        ...
        var pointDatLich = $("#" + strZoneButton + "_DatLich");
        ...
        for (var i = 0; i < data.length; i++) {
            var strCheck = "";
            if (data[i].MAUIMPORT_MA && data[i].MAUIMPORT_MA.length > 14) {
                strCheck = data[i].MAUIMPORT_MA.substring(0, 14).toUpperCase();
            }
            let htmlSuaFile = '';
            if (data[i].SUAFILEBAOCAO) htmlSuaFile = '<a style="margin-left: auto; cursor: pointer" id="' + data[i].ID + '" class="btnEdit_MauBaoCao">Sửa mẫu</a>'
            switch (strCheck) {
                case "REPORTALLTABLE": html += '<li><a class="btnReportAllTable" name="' + data[i].MAUIMPORT_MA + '" duongdan="' + edu.util.returnEmpty(data[i].MAUIMPORT_DUONGDAN) + '" href="#"> ' + (i + 1) + '. ' + data[i].MAUIMPORT_TENFILEMAU + ' - <i class="fa fa-eye" style="color:red"></i> </a></li>';
                    break;
                case "REPORTALLINPUT": html += '<li><a class="btnReportAllInput" name="' + data[i].MAUIMPORT_MA + '" href="#"> ' + (i + 1) + '. ' + data[i].MAUIMPORT_TENFILEMAU + ' - <i class="fa fa-eye" style="color:red"></i> </a></li>';
                    break;
                case "IMPORTALLINPUT": htmlimport += '<li><a class="btnImportAllInput" href="#"> ' + (i + 1) + '. ' + data[i].MAUIMPORT_TENFILEMAU + ' - <i class="fa fa-cloud-upload" style="color:red"></i> </a></li>';
                    break;
                case "IMPORTWITHPROC": htmlimport += '<li><a class="btnImportWithProce" name="' + data[i].MAUIMPORT_MA + '" title="' + data[i].MAUIMPORT_TENFILEMAU + '" href="#"> ' + (i + 1) + '. ' + data[i].MAUIMPORT_TENFILEMAU + '</a></li>';
                    break;
                default: html += '<li style="display: flex"><a class="btnBaoCao_LHD" name="' + data[i].MAUIMPORT_MA + '" duongdan="' + edu.util.returnEmpty(data[i].MAUIMPORT_DUONGDAN) + '"  href="#"> ' + (i + 1) + '. ' + data[i].MAUIMPORT_TENFILEMAU + ' </a> ' + htmlSuaFile +'</li>';
            }
        ...
        if (html != "") {
            var row = '';
            row += '<button class="btn btn-success dropdown-toggle" type="button" data-toggle="dropdown">';
            row += '<i class="fa fa fa-file-excel-o"></i> Xuất báo cáo <span class="caret"></span>';
            row += '</button>';
            row += '<ul class="dropdown-menu form-container" style="width: unset !important">';
            row += html;
            row += '</ul>';
            strPoint.html(row);
            strPoint.show();
        ...
        if (pointDatLich.length > 0) {
            var row = '';
            row += '<button class="btn btn-default dropdown-toggle" type="button" data-toggle="dropdown">';
            row += '<i class="fa fa-calendar-check-o"></i> Đặt tự động <span class="caret"></span>';
            row += '</button>';
            row += '<ul class="dropdown-menu form-container" style="width: unset !important">';
            row += html;
            row += '</ul>';
            pointDatLich.html(row);
            pointDatLich.show();
        }
        else {
            if (pointDatLich.html() == "") {
                pointDatLich.hide();
            }
        };
        ...
        if (htmlimport != "") {
            var rowimport = '';
            rowimport += '<button class="btn btn-success dropdown-toggle" type="button" data-toggle="dropdown">';
            rowimport += '<i class="fa fa-cloud-upload"></i> Import <span class="caret"></span>';
            rowimport += '</button>';
            rowimport += '<ul class="dropdown-menu form-container" style="width: unset !important">';
    ...
    report: function (strLoaiBaoCao, strDuongDan, callback, strTable_Id, checkBaoCao, bDatLich, callback2) {
        ...
        if (checkBaoCao && checkBaoCao.XEMFILE) addKeyValue("saveFile", checkBaoCao.XEMFILE);
        if (bDatLich) {
            if (arrTuKhoa.indexOf("saveFile") == -1) addKeyValue("saveFile", "xlsx");
            addKeyValue("CHAYTUDONG", "TUDONG");
        }
        ...
        if (jQuery.type(callback2) == "function") {
            var check = callback2(addKeyValue);
            if (check == false) return;
        }
        //không sửa ở đây
            ...
            success: function (data) {
                if (data.Success) {
                    ...
                    if (!edu.util.checkValue(strBaoCao_Id)) {
                        ...
                        if (bDatLich) {
                            let htmlLich = '';
                            htmlLich += '<div class="row">';
                            htmlLich += '<div class="col-sm-3">';
                            htmlLich += '<label style="font-weight: normal"><span class="lang" key="">Chu kỳ chạy</span></label>';
                            htmlLich += '</div>';
                            htmlLich += '<div class="col-sm-9"><input style="display: none" />';

                            htmlLich += '<select id="dropChuKyBaoCao" class="select-opt">';

                            htmlLich += '<option value="HANGNGAY">Hàng ngày</option>';
                            htmlLich += '<option value="CUOINGAY">Cuối ngày</option>';
                            htmlLich += '<option value="DAUTUAN">Đầu tuần</option>';
                            htmlLich += '<option value="CUOITUAN">Cuối tuần</option>';
                            htmlLich += '<option value="DAUTHANG">Đầu tháng</option>';
                            htmlLich += '<option value="CUOITHANG">Cuối tháng</option>';
                            htmlLich += '</select > ';
                            htmlLich += '</div>';
                            htmlLich += '<div class="clear"></div>';
                            htmlLich += '</div>';
                            edu.system.confirm(htmlLich, "Đặt lịch");
                            $("#btnYes").click(function (e) {
                                //$('#myModalAlert').modal('hide');
                                let strReport = strDuongDan ? strDuongDan : me.rootPathReport;
                                if (strReport && strReport.indexOf("http") == -1) strReport = me.strhost + strReport;
                                me.save_ChuKyDatLich(strBaoCao_Id, strReport)
                            });
                            return;
                        }
                        var url_report = me.rootPathReport + "?id=" + strBaoCao_Id;
                        console.log(strDuongDan);
                        if (edu.util.checkValue(strDuongDan) && strDuongDan != "undefined") {
                            url_report = strDuongDan + "?id=" + strBaoCao_Id;
                            console.log(url_report);
                        }
                        if (strDuongDan && strDuongDan.indexOf("http") == -1) url_report = me.strhost + url_report;
                        console.log(url_report);
                        //window.open(url_report, '_blank').focus();
                        if (checkBaoCao && checkBaoCao.XEMFILE) {

                            console.log("Xem file");
                            getList_BaoCao(url_report);
                        } else location.href = url_report;
                    }
                }
                else {
    ...
    reportAllTable: function (strTable_Id, strLoaiBaoCao, strDuongDan, callback) {
        ...
        //console.log(document.getElementById("zonereportalltable"));
                ...
                for (var j = 0; j < head[i].cells.length; j++) {
                    ...
                    if (cell.style.display != "none") {
                        ...
                        for (var k = 0; k < arrFile.length; k++) {
                            strValue += ";" + edu.system.rootPathUpload + "/" + $(arrFile[k]).attr("name");
                        }
        ...
        function Xeload(arrData, strRow_Id) {
            for (var i = 0; i < arrData.length; i++) {
                ...
                if (strRow_Id) {
                    ...
                    console.log("strRow_Id:" + strRow_Id);
                    console.log("strPoint_Id:" + pointNode.id);
            ...
            for (var i = 0; i < lhang; i++) {
                for (var j = 0; j < head[i].cells.length; j++) {
                    ...
                    if (cell.style.display != "none") {
                        ...
                        for (var k = 0; k < arrFile.length; k++) {
                            strValue += "\n" + edu.system.rootPathUpload + "/" + $(arrFile[k]).attr("name");
                        }
                        if (strValue != "") strValue = strValue.substring(1);
                        else strValue = $(cell).text();
                        if (strTempId != "") {
    ...
    showReportAndImportTable_User: function (bUpload) {
        ...
        row += '<div id="zoneImportAllInput"><div class="col-sm-4" >- Thực hiện import: </div><div class="col-sm-8"><div id="importAllTable"></div></div></div>';
        row += '</div><div class="clear"></div>';
    ...
    showImportChung: function (strTenHienThi, strMaDanhMuc) {
        ...
        row += '</div><div class="clear"></div>';
        // Panel tiến trình import (2026-09-17). Id có hậu tố 1 để không đụng bản V2 bên dưới.
        // Bản này import tự chạy ngay sau khi upload xong, trước đó không có phản hồi nào.
        row += '<div id="zoneImportProgress1" style="display:none;margin:12px 0;padding:14px 16px;border:1px solid #dbe4f3;border-radius:8px;background:#f8fbff">';
        row += '<div style="display:flex;align-items:center;margin-bottom:10px">';
        row += '<i class="fa fa-spinner fa-spin" style="color:#2563eb;font-size:16px;margin-right:10px"></i>';
        row += '<b id="lblImportProgress1" style="color:#1e40af">Đang import...</b>';
        row += '</div>';
        row += '<div class="progress" style="height:18px;margin:0;background:#e8eefb;border-radius:9px;overflow:hidden">';
        row += '<div class="progress-bar progress-bar-striped active" role="progressbar" style="width:100%;background-color:#2563eb"></div>';
        row += '</div>';
        row += '<div style="margin-top:8px;font-size:12.5px;color:#64748b">Máy chủ đang xử lý toàn bộ file. Vui lòng không đóng cửa sổ này.</div>';
        row += '<div id="lblImportNote1" style="margin-top:6px;font-size:12.5px;color:#b45309"></div>';
        row += '</div>';
        row += '<div id="zoneImportMsg1" style="display:none;margin:10px 0;padding:11px 14px;border-radius:6px;font-size:13.5px"></div>';
        edu.system.alert(row);
        ...
        function _stopImportProgress() {
            ...
            $("#lblImportNote1").html("");
            $("#zoneImportProgress1").hide();
        }

        function GetDuLieuDanhMuc(a, strPath) {
            if (_bImportRunning) return;
            // Callback của uploadImport chạy cả khi người dùng GỠ file (strPath rỗng) — 2026-09-17
            ...
            if (strMaDanhMuc === undefined || strMaDanhMuc === "") {
                // Thiếu return ở đây khiến import bị gọi 2 lần — 2026-09-17
                ImportData(obj_list);
                return;
            }
            me.makeRequest({
                success: function (data) {
                    // lKeyVal khai báo ngoài if: Success=false thì trước đây obj_list.lKeyVal = undefined
                    var lKeyVal = [];
                    if (data.Success) {
        ...
        function ImportData(obj_list) {
            ...
            edu.system.makeRequest({
                ...
                error: function (er) {
                    // Trước đây đổ thẳng JSON.stringify(er) ra alert — 2026-09-17
                    _iImportRetry++;
                    if (_iImportRetry <= 2) {
                        $("#lblImportNote1").html('<i class="fa-solid fa-rotate-right" style="margin-right:6px"></i>Kết nối lỗi, đang thử lại lần ' + _iImportRetry + "/2...");
                        setTimeout(function () {
                            ImportData(obj_list);
                        }, 1000);
                        return;
    ...
    showImportChungV2: function (strTenHienThi, strMaDanhMuc) {
        ...
        row += '</div>';
        row += '<div class="clear"></div>';
        // Panel tiến trình import (2026-09-17): trước đây bấm "Thực hiện import" không có phản hồi nào
        // ngoài overlay spinner toàn cục, người dùng tưởng máy treo rồi một lúc mới thấy bảng lỗi.
        // Panel đặt ngay trong modal nên không phụ thuộc z-index của #overlay.
        row += '<div id="zoneImportProgress" style="display:none;margin:10px 0;padding:14px 16px;border:1px solid #dbe4f3;border-radius:8px;background:#f8fbff">';
        row += '<div style="display:flex;align-items:center;margin-bottom:10px">';
        row += '<i class="fa fa-spinner fa-spin" style="color:#2563eb;font-size:16px;margin-right:10px"></i>';
        row += '<b id="lblImportProgress" style="color:#1e40af">Đang import...</b>';
        row += '</div>';
        row += '<div class="progress" style="height:18px;margin:0;background:#e8eefb;border-radius:9px;overflow:hidden">';
        row += '<div class="progress-bar progress-bar-striped active" id="barImportProgress" role="progressbar" style="width:100%;background-color:#2563eb"></div>';
        row += '</div>';
        row += '<div style="margin-top:8px;font-size:12.5px;color:#64748b">Máy chủ đang xử lý toàn bộ file. Vui lòng không đóng cửa sổ này.</div>';
        // Dòng riêng cho thông báo retry — nếu ghi chung vào #lblImportProgress sẽ bị đồng hồ đè mỗi giây
        row += '<div id="lblImportNote" style="margin-top:6px;font-size:12.5px;color:#b45309"></div>';
        row += '</div>';
        // Banner thông báo inline. KHÔNG dùng edu.system.alert ở đây: alert của BS3 chồng lên
        // #modalBaoCao đang mở, khi tắt alert nó gỡ body.modal-open làm modal import tự đóng theo.
        row += '<div id="zoneImportMsg" style="display:none;margin:10px 0;padding:11px 14px;border-radius:6px;font-size:13.5px"></div>';
        ...
        function _showImportMsg(strNoiDung, bLoi) {
            var strMau = bLoi === false
                ? 'background:#ecfdf5;border:1px solid #a7f3d0;color:#065f46'
                : 'background:#fef2f2;border:1px solid #fecaca;color:#991b1b';
            var strIcon = bLoi === false ? 'fa-circle-check' : 'fa-triangle-exclamation';
            $("#zoneImportMsg").attr("style", "display:block;margin:10px 0;padding:11px 14px;border-radius:6px;font-size:13.5px;" + strMau)
                .html('<i class="fa-solid ' + strIcon + '" style="margin-right:8px"></i>' + strNoiDung);
        }
        ...
        function _startImportProgress() {
            var iTong = 0;
            try {
                var dtSheet = me.dtImport[$("#dropSearch_BangA").val()];
                if (dtSheet && dtSheet.length) iTong = dtSheet.length;
            } catch (ex) { }
            var dStart = new Date().getTime();
            $("#lblImportNote").html("");
            $("#zoneImportProgress").show();
            var btn = $("#btnThucHienImport");
            if (!btn.attr("data-html-goc")) btn.attr("data-html-goc", btn.html());
            ...
            var fnTick = function () {
                ...
                var strPhut = ("0" + Math.floor(iGiay / 60)).slice(-2);
                var strGiay = ("0" + (iGiay % 60)).slice(-2);
                $("#lblImportProgress").html("Đang import" + (iTong > 0 ? " " + iTong + " dòng" : "") + " — đã chạy " + strPhut + ":" + strGiay);
            };
            fnTick();
            if (_timerImport) clearInterval(_timerImport);
            _timerImport = setInterval(fnTick, 1000);
        ...
        function _stopImportProgress() {
            ...
            var strHtmlGoc = btn.attr("data-html-goc");
            if (strHtmlGoc) btn.html(strHtmlGoc);
        }

        $("#btnThucHienImport").click(function (e) {
            if (e && e.preventDefault) e.preventDefault();
            if ($(this).hasClass("disabled")) return;
            // Chặn gọi API khi chưa có file / chưa chọn sheet — trước đây vẫn bắn request rồi mới lỗi
            var strPathFile = edu.util.getValById("importToCheck");
            if (strPathFile === undefined || strPathFile === null || strPathFile === "") {
                _showImportMsg("Vui lòng chọn file import trước khi thực hiện.");
                return;
            }
            var strSheetVal = $("#dropSearch_BangA").val();
            if (strSheetVal === undefined || strSheetVal === null || strSheetVal === "") {
                _showImportMsg("Vui lòng chọn sheet import.");
                return;
            }
            _hideImportMsg();
            _iImportRetry = 0;
        ...
        function ImportData(obj_list) {
            ...
            edu.system.makeRequest({
                success: function (data) {
                    // Tắt thanh tiến trình trước khi dựng bảng lỗi, vì edu.system.alert bên dưới
                    // mở modal khác đè lên — 2026-09-17
                    _stopImportProgress();
                    if (data.Success) {
                        data = data.Data;
                        var arrDataLoi = [];
                ...
                error: function (er) {
                    // Trước đây retry vô hạn mỗi 1 giây, không đếm lần và không báo gì:
                    // người dùng chỉ thấy màn hình đứng im rồi bất chợt hiện lỗi — 2026-09-17
                    _iImportRetry++;
                    if (_iImportRetry <= 2) {
    ...
    sortTable_Master: function (strTableId) {
        ...
        function sortTable(table, column) {
            ...
            //console.log(node);
            //console.log(document.getElementById("myList1"));
            ...
            // console.log(document.getElementById('tbldata_BLCS_LoaiKhoan2'));
            //$("#" + table + " tbody").html('');
    ...
    actionRowSpan: function (strTableId, arrCol) {
        ...
        function nextRowSpan(icol, iBatDau, iKetThuc, arrChild) {
            ...
            var arr = arrChild;//
            if (arr === undefined || arr.length < 1) return;//arrChild không tồn tại
    ...
    getList_RangBuoc: function (strTableDB, strId) {
        ...
        //--Edit
        var obj_list = {
            'action': 'CMS_RangBuoc_Cot/LayTTRangBuocCot',
    ...
    nodeChat: function (urlNode) {
        ...
        //$("#zoneAllMessage").show();
        try {
            console.log(typeof io !== "undefined");
            if (typeof io !== "undefined") {
                console.log("Vao io", urlNode);
                ...
                if (me.appCode.toUpperCase() == "APISCMS") $("#zoneAllMessage").parent().show();
                var socket = io(urlNode);
                var objUser = {
                    UserName: $(".user-menu .hidden-xs").text(),
                    UserID: me.userId,
                    UserMa: '',
                    UserServer: me.objApi.DonVi,
                    UserModule: me.appCode,
                    strChucNang_Id: me.strChucNang_Id,
                    UserAvatar: $(".user-menu .user-image").attr("src")
                };
                me.socket = socket;
                ...
                $("#btnReload").click(function () {
                    ...
                    console.log("reload", arrChecked_Id);
        ...
        function genMessage(data) {
            ...
            var classPosition = "";
            var show = "";
            if (data.UserID == me.userId) { classPosition = "right"; show = "pull-right"; }
            var rMess = "";
            rMess += '<div class="direct-chat-msg ' + classPosition + '" title="' + data.date + '">';
            rMess += '<div class="direct-chat-info clearfix">';
            rMess += '<span class="direct-chat-name ' + show + '">' + data.UserName + '</span>';
            //rMess += '<span class="direct-chat-timestamp pull-left">23 Jan 2:05 pm</span>';
            rMess += '</div>';
            rMess += '<img class="direct-chat-img" src="' + data.UserAvatar + '" alt="' + data.UserMa + '">';
            ...
            rMess += '</div>';
            var pointChat = $("#zoneChat #" + data.roomId + " .direct-chat-messages");
            pointChat.append(rMess);
    ...
    showBaoCao: function () {
        ...
        row += '<p class="titleSearchContent" id="colthongbao" data-toggle="collapse" data-target="#showthongbao">';
        row += '<i class="fa fa-window-restore">';
        row += '</i> <span class="lang" key="">Nâng cao</span>';
        row += '</p>';
        row += '</div>';

        row += '<div class="area-search"><div id="showthongbao" class="inputSearch collapse in" >';
        ...
        $("#btnThucHienCheck").click(function () {
            ...
            $("#" + strBangB + " tbody tr").each(function () {
                ...
                $(pGH).attr('checked', false);
                $(pGH).prop('checked', false);
            })
            $("#tblBangA tbody tr").each(function () {
                let tempGT = $(this).find("td:eq(" + iCotA + ")").text().trim();
                $("#" + strBangB + " tbody tr").each(function () {
                    let tempGH = $(this).find("td:eq(" + iCotB + ")").text().trim();
        ...
        $("#colthongbao").trigger("click");
    },
    showCot: function (strTable_Id, strDrop_Id) {
        var temp = "";
        let iThu = -1;
        $("#" + strTable_Id + " thead tr:eq(0) th").each(function () {
            let checktext = this.innerText;
            iThu++;
            if (checktext.toLowerCase().trim() == "stt" || checktext.toLowerCase().trim() == "thứ tự" || checktext.toLowerCase().trim() == "") return;
            temp += '<option value="' + iThu + '">' + checktext + '</option>';
        });
    ...
    save_ChuKyDatLich: function (strBaoCao_Id, strDuongDanBaoCao) {
        var me = this;
        var obj_notify = {};
        //--Edit
        var obj_save = {
            'action': 'TC_ThuChi2_MH/FSkkLB4DIC4CIC4eFRUeFTQFLi8mHg0oIikP',
            'func': 'PKG_TAICHINH_THUCHI2.Them_BaoCao_TT_TuDong_Lich',
            'iM': edu.system.iM,
            'strBaoCao_Id': strBaoCao_Id,
            'strNgayChay': edu.system.getValById('dropChuKyBaoCao'),
            'strDuongDanBaoCao': strDuongDanBaoCao,
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    me.save_DatTuDong(strBaoCao_Id)
                }
                else {
                    edu.system.alert(data.Message);
                }
    ...
    save_DatTuDong: function (strBaoCao_Id) {
        var me = this;
        var obj_notify = {};
        //--Edit
        var obj_save = {
            'action': 'TC_ThuChi2_MH/BSA1FTQFLi8mAyAuAiAu',
            'func': 'PKG_TAICHINH_THUCHI2.DatTuDongBaoCao',
            'iM': edu.system.iM,
            'strBaoCao_Id': strBaoCao_Id,
            'strNguoiThucHien_Id': edu.system.userId,
        };
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    edu.system.alert("Đặt tự động thành công!");
                }
                else {
                    edu.system.alert(data.Message);
                }
    ...
    save_MauFileBaoCao: function () {
        ...
        if (obj_save.strId) {
            obj_save.action = 'CM_Report/UPDATE_BC_NGUOIDUNG_TEMPLATE';
        }
        //default
        edu.system.makeRequest({
            success: function (data) {
    ...
    getList_MauFileBaoCao: function () {
        var me = this;
        var obj_save = {
            'action': 'CM_Report/LayDSBAOCAO_NGUOIDUNG_TEMPLATE',
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    me["dtMauFileBaoCao"] = data.Data;
                    me.genTable_MauFileBaoCao(data.Data);
                }
                else {
    ...
    delete_MauFileBaoCao: function (Ids) {
        var me = this;
        //--Edit
        var obj_save = {
            'action': 'CM_Report/XOA_BC_NGUOIDUNG_TEMPLATE',
            'strId': Ids,
            'strNguoiThucHien_Id': edu.system.userId,
        };
        //default
    ...
    genTable_MauFileBaoCao: function (data, iPager) {
        ...
        var jsonForm = {
            ...
            aaData: data,
            //bPaginate: {
            //    strFuntionName: "main_doc.DonGia.getList_MauFileBaoCao()",
            //    iDataRow: iPager
            //},
            colPos: {
                center: [0],
            ...
            aoColumns: [
                ...
                {
                    "mRender": function (nRow, aData) {
                        return '<span><a class="btn btn-default btnDownLoadBaoCao" id="' + aData.ID + '" title="Sửa"><i class="fa fa-cloud-download"></i></a></span>';
                    }
                },
                {
                    "mRender": function (nRow, aData) {
                        ...
                        if (strDuongDan) {
                            ...
                            strDuongDan = strDuongDan.substring(strDuongDan.indexOf('Upload/File'));
                            strDuongDan = result + '/' + strDuongDan;
                            return '<span><a class="btn btn-default" href="' + strDuongDan + '" title="Sửa"><i class="fa fa-cloud-download"></i></a></span>';
