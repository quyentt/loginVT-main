...
ChuyenDuLieu.prototype = {
    ...
    init: function () {
        ...
        edu.system.loadToCombo_DanhMucDuLieu("QLTC.NVAP", "dropNghiepVu_CDL", "Chọn nghiệp vụ áp dụng");
        $('#dropHeDaoTao_CDL').on('select2:select', function () {
            var strHeHaoTao_Id = edu.util.getValById("dropHeDaoTao_CDL");
            me.getList_KhoaDaoTao(strHeHaoTao_Id);
        });
        $('#dropKhoaDaoTao_CDL').on('select2:select', function () {
            var strKhoaDaoTao_Id = edu.util.getValById("dropKhoaDaoTao_CDL");
            var strChuongTrinhDaoTao_Id = edu.util.getValById("dropChuongTrinhDaoTao_CDL");
            me.getList_LopQuanLy(strKhoaDaoTao_Id, strChuongTrinhDaoTao_Id);
        });
        $('#dropLopQuanLy_CDL').on('select2:select', function () {
            me.getList_SinhVien();
        });
        $('#dropThoiGianDaoTao_TP').on('select2:select', function () {
            me.getList_KeHoachDangKy();
        });
        $('#dropNghiepVu_CDL').on('select2:select', function () {
            me.getList_LoaiKhoan();
        });
        /*------------------------------------------
        ...
        -------------------------------------------*/
        //var obj_Queue = {
        //    strLoaiNhiemVu: "CHUYENKETOANTUDONG" + $("#dropNghiepVu_CDL").val()
        //}
        ...
        //$("#tblTaskBar_ChuyenDuLieu").delegate('.btnStart', 'click', function () {
        //    var strId = this.id;
        ...
        //    }
        //});
        //$("#tblTaskBar_ChuyenDuLieu").delegate('.btnCancle', 'click', function () {
        //    var strId = this.id;
        ...
        $("#tblTaskBar_ChuyenDuLieu").delegate("#btnXemDanhSach", "click", function () {
            me.getList_NienChe();
            me.getList_TinChi();
        });
        /*------------------------------------------
        ...
        $("#tblTinChi_ChuyenDuLieu").delegate(".btnChiTiet", "click", function () {
        ...
        //Xuất báo cáo
        edu.system.getList_MauImport("zonebtnBaoCao_CKT", function (addKeyValue) {
            addKeyValue("strHeDaoTao", edu.util.getValById("dropHeDaoTao_CDL"));
            addKeyValue("strKhoaDaoTao", edu.util.getValById("dropKhoaDaoTao_CDL"));
            addKeyValue("strThoiGianDaoTao", edu.util.getValCombo("dropThoiGianDaoTao_CDL"));
            addKeyValue("strKhoanThu", edu.util.getValById("dropKhoanThu_CDL"));
            addKeyValue("strNghiepVu", edu.util.getValById("dropNghiepVu_CDL"));
        });
        $(document).delegate(".ckbDSTrangThaiSV_CKT_ALL", "click", function (e) {
            e.stopImmediatePropagation();
            var checked_status = this.checked;
            $(".ckbDSTrangThaiSV_CKT").each(function () {
        ...
        $('#dropThoiGianDaoTao_CDL').on('select2:select', function () {
            me.getList_KeHoachDangKy();
        });
    },
    /*------------------------------------------
    ...
    TaoHangDoi_ChuyenDuLieu_TuDong: function () {
        ...
        var obj_save = {
            ...
            'strTaiChinh_CacKhoanThu_Id': edu.util.getValById('dropKhoanThu_CDL'),
            'strDaoTao_ThoiGianDaoTao_Id': edu.util.getValCombo('dropThoiGianDaoTao_CDL'),
            'strDaoTao_HeDaoTao_Id': edu.util.getValById('dropHeDaoTao_CDL'),
            'strDaoTao_KhoaDaoTao_Id': edu.util.getValById('dropKhoaDaoTao_CDL'),
            'strNghiepVuApDung_Id': edu.util.getValById('dropNghiepVu_CDL'),
            'strDangKy_KeHoachDangKy_Id': edu.util.getValById('dropKeHoachDangKy_TP'),
            'strTrangThaiNguoiHoc_Id': edu.extend.getCheckedCheckBoxByClassName('ckbDSTrangThaiSV_CKT').toString(),
            'strNguoiThucHien_Id': edu.system.userId,
            'strQLSV_NguoiHoc_Id': arrChecked_Id.toString(),
            'strDaoTao_LopQuanLy_Id': edu.util.getValCombo('dropLopQuanLy_CDL'),
        };
        //default
    ...
    proSeqItem_HangDoi: function (obj, strHangDoi_Id) {
        var me = main_doc.ChuyenDuLieu;
        me.countQueue++;
        var renderPlace_Progress = "probar_ChuyenDuLieu" + strHangDoi_Id
        return new Promise(function (resolve, reject) {
            var obj_Task = {
                strHangDoi_Id: strHangDoi_Id,
    ...
    genHTML_HangDoi: function (data, iPager) {
        ...
        var obj = {};
        var html = "";
        var place = "#tblTaskBar_ChuyenDuLieu tbody";
        var strHangDoi_Id = "";
        var iTongDuLieu = 0;
        ...
        if (data.length > 0) {
            for (var i = 0; i < data.length; i++) {
                ...
                if (iTongDuLieu != iTongDuLieu_HoanThanh) {
                    ...
                    html += '<div id="probar_ChuyenDuLieu' + strHangDoi_Id + '">';
                    html += '</div>';
                    html += '<div id="action_ChuyenDuLieu" class="pull-right">';
                    ...
                    obj.renderPlace = "probar_ChuyenDuLieu" + strHangDoi_Id;
    ...
    genHistory_HangDoi: function (data, iPager) {
        var me = this;
        var jsonForm = {
            strTable_Id: "tblHistory_ChuyenDuLieu",
            aaData: data,
            sort: true,
            colPos: {
                left: [1, 2],
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    edu.system.alert("T: " + data.Message, "w");
                }
                edu.system.endLoading();
            },
            error: function (er) {
    ...
    getList_KeHoachDangKy: function () {
        ...
        var obj_list = {
            ...
            'type': 'GET',
            'strDaoTao_ThoiGianDaoTao_Id': edu.util.getValById('dropThoiGianDaoTao_CDL'),
            'strNguoiThucHien_Id': edu.system.userId,
        };
    ...
    cbGenCombo_KhoaDaoTao: function (data) {
        ...
        var obj = {
            ...
            renderPlace: ["dropKhoaDaoTao_CDL"],
            type: "",
    ...
    loadToCombo_ThoiGianDaoTao: function (data) {
        ...
        var obj = {
            ...
            renderPlace: ["dropThoiGianDaoTao_CDL"],
            type: "",
            title: "Chọn học kỳ",
        }
        edu.system.loadToCombo_data(obj);
    ...
    getList_TinChi: function () {
        ...
        var obj_list = {
            ...
            'strDaoTao_ThoiGianDaoTao_Id': edu.util.getValCombo('dropThoiGianDaoTao_CDL'),
            'strHeDaoTao_Id': edu.util.getValById('dropHeDaoTao_CDL'),
            'strKhoaDaoTao_Id': edu.util.getValById('dropKhoaDaoTao_CDL'),
            'strNghiepVuApDung_Id': edu.util.getValById('dropNghiepVu_CDL'),
            'strTaiChinh_CacKhoanThu_Id': edu.util.getValById('dropKhoanThu_CDL'),
            'strNguoiThucHien_Id': edu.system.userId,
    ...
    genTable_TinChi: function (data, iPager) {
        var me = this;
        $("#tblTinChi_ChuyenDuLieu_Tong").html(iPager);
        var strTable_Id = "tblTinChi_ChuyenDuLieu";
        var jsonForm = {
    ...
    genTable_NienChe: function (data, iPager) {
        var me = this;
        $("#tblNienChe_ChuyenDuLieu_Tong").html(iPager);
        var strTable_Id = "tblNienChe_ChuyenDuLieu";
        var jsonForm = {
            strTable_Id: strTable_Id,
            aaData: data,
            colPos: {
    ...
    genList_TrangThaiSV: function (data) {
        ...
        row += '<input style="float: left; margin-right: 5px" type="checkbox" class="ckbDSTrangThaiSV_CKT_ALL" checked="checked"/>';
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            row += '<div class="col-lg-3 checkbox-inline user-check-print">';
            row += '<input checked="checked" style="float: left; margin-right: 5px" type="checkbox" id="' + data[i].ID + '" class="ckbDSTrangThaiSV_CKT" title="' + data[i].TEN + '"' + strcheck + '/>';
            row += '<span><p>' + data[i].TEN + '</p></span>';
            row += '</div>';
        }
        $("#DSTrangThaiSV_CKT").html(row);
    },
    ...
    getList_SinhVien: function (strDanhSach_Id) {
        ...
        var obj_list = {
            ...
            'pageIndex': 1,
            'pageSize': 100000,
            'strChuongTrinh_Id': edu.util.getValById('dropSearch_ChuongTrinh_CDL'),
            'strKhoaDaoTao_Id': edu.util.getValById('dropKhoaDaoTao_CDL'),
            'strHeDaoTao_Id': edu.util.getValById('dropHeDaoTao_CDL'),
            'strLopHoc_Id': edu.util.getValById('dropLopQuanLy_CDL'),
