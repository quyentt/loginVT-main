...
HeThongPhieuThu.prototype = {
    ...
    popover_SBL: function (strHS_Id, point) {
        ...
        row += '<tbody>';
        row += '<tr>';
        row += '<td><i class="fa fa-empire colorcard"></i> <span class="lang" key="">Số</span></td><td>: ' + edu.util.checkEmpty(data.SOPHIEUTHU) + '</td>';
        row += '</tr>';
        row += '<tr>';
        row += '<td><i class="fa fa-star colorcard"></i> <span class="lang" key="">Năm</span></td><td>: ' + edu.util.checkEmpty(data.HETHONGPHIEUTHU_NAM) + '</td>';
        row += '</tr>';
        row += '<tr>';
