...
LopHocPhan.prototype = {
    ...
    init: function () {
        ...
        $("#btnSave_Tinh").click(function () {
            var arrThem = [];
            $("#tblLopHocPhan .inputMucPhi").each(function () {
                var strValue = $(this).attr("name");
                var x = $(this).val();
                if (x != strValue) arrThem.push(this.id.replace(/txtSoTien/g, ''));
            })
            if (arrThem.length == 0) {
            ...
            arrThem.forEach(e => me.save_TinhTien(e));
        });
    },
    ...
    genTable_LopHocPhan: function (data, iPager) {
        ...
        var jsonForm = {
            ...
            aoColumns: [
                ...
                {
                    "mRender": function (nRow, aData) {
                        return '<input type="text" class="inputMucPhi form-control"  id="txtSoTien' + aData.ID + '" value="' + edu.util.returnEmpty(aData.TONGSOTIEN) + '"  name="' + edu.util.returnEmpty(aData.TONGSOTIEN) + '" />';
                    }
                },
                {
                    "mRender": function (nRow, aData) {
    ...
    save_TinhTien: function (strDaoTao_LopHocPhan_Id) {
        var me = this;
        var obj_notify = {};
        //--Edit
        var obj_save = {
            ...
            'type': 'POST',
            'strDaoTao_LopHocPhan_Id': strDaoTao_LopHocPhan_Id,
            'dTongSoTien': edu.util.getValById('txtSoTien' + strDaoTao_LopHocPhan_Id),
