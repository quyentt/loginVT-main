...
HeSoHocPhan.prototype = {
    ...
    init: function () {
        ...
        $("#btnSave_HSHP").click(function () {
            //check HocKy, dTongSoTien, KhoanThu, TinhChat, arrHocPhan_Id,....
    ...
    rewrite: function () {
        //reset id
        var me = this;
        var arrId = ["dropHocKy_HSHP", "txtSoTien_HSHP", "dropKhoanThu_HSHP", "dropKieuHoc_HSHP"];
    ...
    save_HeSoHocPhan: function () {
        ...
        var dTongSoTien = edu.util.convertStrToNum($("#txtSoTien_HSHP").val());
    ...
    update_HeSoHocPhan: function (arrId, dTongSoTien, strIds) {

        var me = this;
        var obj_notify = {};
        //format arId ===> [HocKy, HocPhan, LoaiKhoan, KieuHoc]
        var obj_save = {
            'action': 'TC_HocPhan_SoTien/CapNhat',
            'versionAPI': 'v1.0',
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    me.affectUpdate_HeSoHocPhan(strIds, dTongSoTien);
                    //notify alert)timer
                    var obj = {
                        title: "Thông báo",
                    ...
                    edu.system.alert("QLTC_HocPhan_SoTien.CapNhat: " + data.Message);
                }
                edu.system.endLoading();
            },
            error: function (er) {
                edu.system.endLoading();
                edu.system.alert("QLTC_HocPhan_SoTien.CapNhat (er): " + JSON.stringify(er));
            },
            type: "POST",
            action: obj_save.action,
            versionAPI: obj_save.versionAPI,
    ...
    delete_HeSoHocPhan: function (arrId) {
        var me = this;
        var obj_notify = {};
        var arrIds = edu.util.convertStrToArr(arrId, "_");
        //format arId ===> [HocKy, HocPhan, LoaiKhoan, KieuHoc]
        var obj_save = {
            'action': 'TC_HocPhan_SoTien/Xoa_TaiChinh_HocPhan_SoTien',
            'versionAPI': 'v1.0',

            'strPhamViApDung_Id': edu.util.getValById("dropChuongTrinhDaoTao_HSHP"),
            'strPhanCapApDung_Id': "",
            'strNgayApDung': "",
            'strDaoTao_HocPhan_Id': arrIds[1],
            'strDaoTao_ThoiGianDaoTao_Id': arrIds[0],
            'strNguoiThucHien_Id': edu.system.userId,
            'strDiem_KieuHoc_Id': arrIds[3],
            'strTaiChinh_CacKhoanThu_Id': arrIds[2],
        };
        //default
        edu.system.beginLoading();
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    me.affectDelete_HeSoHocPhan(arrId);
                    //remark and update new value on HTML
                    var obj = {
    ...
    getList_HeSoHocPhan: function () {
        ...
        var pageIndex = 1;
        var pageSize = 10000;
        var strDaoTao_ToChucChuongTrinh_Id = edu.util.getValById("dropChuongTrinhDaoTao_HSHP");
    ...
    genTbody_HeSoHocPhan: function (dtHocPhan, dtHeSoHocPhan, dtKhoanThu, dtKieuHoc) {
        ...
        function genTbody(arrHocKy, arrHocPhan, arrHeSoHocPhan, arrLoaiKhoan) {
            ...
            for (var hk = 0; hk < arrHocKy.length; hk++) {//1.loop HocKy
                ...
                //gen data dTongSoTien for each HocKy_Id
                for (var lk = 0; lk < arrLoaiKhoan.length; lk++) {//2.loop LoaiKhoan
                    for (var kh = 0; kh < dtKieuHoc.length; kh++) {//3.loop KieuHoc
                        ...
                        for (var dt = 0; dt < arrHeSoHocPhan.length; dt++) {//4.loop HeSoHocPhan
                            if (arrHocKy[hk].ID == arrHeSoHocPhan[dt].DAOTAO_THOIGIANDAOTAO_ID
                                ...
                                //has value
                                dTongSoTien = edu.util.formatCurrency(edu.util.returnZero(arrHeSoHocPhan[dt].TONGSOTIEN));
                                //gen tr
                                tbody += genRow(arrId, dTongSoTien);
                                check = true;
                            }
                        }
                        if (!check) {//no value
        ...
        function genRow(arrId, dTongSoTien) {
            ...
            tbody += '<a id="delete_' + arrId + '" class="btn btn-default btn-circle btnDel_HSHP" title="Xóa"><i class="fa fa-times-circle color-active"></i></a>';
            tbody += '</span>';
            tbody += '</span>';
