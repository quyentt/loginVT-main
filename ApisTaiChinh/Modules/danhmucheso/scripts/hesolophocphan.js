...
--API URL: QLTC_LopHocPhan_HeSo
--Note:
--Updated by:
--Date of updated:
----------------------------------------------*/
function HeSoLopHocPhan() { };
HeSoLopHocPhan.prototype = {
    valid_HSLHP: [],
    html_HSLHP: {},
    input_HSLHP: {},
    strHocPhan_Id: '',
    ...
    init: function () {
        ...
        $(".btnClose").click(function () {
            edu.util.toggle_overide("zone-hsph", "zone_list_hslhp");
        });
        $(".btnAddNew").click(function () {
            me.rewrite();
            edu.util.toggle_overide("zone-hsph", "zone_input_hslhp");
        });
        $("#txtKeyword_HSLHP").focus();
        /*------------------------------------------
		--Discription: [1] Action TimKiem HeSoLopHocPhan
		-------------------------------------------*/
        $('#dropHeDaoTao_HSLHP').on('select2:select', function () {
            var strHeHaoTao_Id = edu.util.getValById("dropHeDaoTao_HSLHP");
            me.getList_KhoaDaoTao(strHeHaoTao_Id);
            me.getList_ChuongTrinhDaoTao("");
            
        });
        $('#dropKhoaDaoTao_HSLHP').on('select2:select', function () {
            var strKhoaHoc_Id = edu.util.getValById("dropKhoaDaoTao_HSLHP");
            me.getList_ChuongTrinhDaoTao(strKhoaHoc_Id);
            
        });
        $('#dropChuongTrinhDaoTao_HSLHP').on('select2:select', function () {
            var strChuongTrinh_Id = edu.util.getValById("dropChuongTrinhDaoTao_HSLHP");
            if (strChuongTrinh_Id) {
                edu.system.beginLoading();
                //1. call db
                me.getList_LopHocPhan(strChuongTrinh_Id);
                //2.hide box-search
                //edu.util.toggle("box-sub-search");
                //3. gen path selected
                me.genPathSelect();
                //4. trigger ----> dropChuongTrinhDaoTao_Form_HSLHP on modal
                $("#dropChuongTrinhDaoTao_Form_HSLHP").val(strChuongTrinh_Id).trigger("change");
                //5. call HocPhan
        ...
        $("#btnRefresh_HSLHP").click(function () {
            if (edu.util.checkValue(me.strHocPhan_Id)) {
                me.getList_HeSoLopHocPhan();
            }
            else {
                edu.system.alert("Vui lòng chọn Học phần để thao tác!", "w");
            }
        });
        $('#dropChuongTrinhDaoTao_Form_HSLHP').on('select2:select', function () {
            var strChuongTrinh_Id = edu.util.getValById("dropChuongTrinhDaoTao_Form_HSLHP");
            if (edu.util.checkValue(strChuongTrinh_Id)) {
                me.getList_HocPhan_OnModal(strChuongTrinh_Id);
            }
        });
        /*------------------------------------------
		--Discription: [1] Action main  HeSoLopHocPhan
		-------------------------------------------*/
        $("#btnSave_HSLHP").click(function () {
        ...
        $("#tbldata_HSLHP").delegate(".btnEdit_HSLHP", "click", function () {
            var selected_id = this.id;
            selected_id = edu.util.cutPrefixId(/edit_/g, selected_id);
            if (edu.util.checkValue(selected_id)) {
                me.openBoxEdit_HeSoLopHocPhan(selected_id);
            }
        });
        $("#tbldata_HSLHP").delegate(".btnUpdate_HSLHP", "keypress", function (e) {
            if (e.which == 13) {
                var selected_id = this.id;
                var strIds = edu.util.cutPrefixId(/txt/g, selected_id);

                if (edu.util.checkValue(strIds)) {
                    var obj = me.closeBoxEdit_HeSoLopHocPhan(strIds);
                    var value = edu.util.convertStrToNum(obj.value);
                    var arrId = obj.arrId;
                    //call update function
                    me.update_HeSoLopHocPhan(arrId, value, strIds);
                }
            }
        });
        $("#tbldata_HSLHP").delegate(".btnDel_HSLHP", "click", function () {
            var selected_id = this.id;
            selected_id = edu.util.cutPrefixId(/delete_/g, selected_id);
            if (edu.util.checkValue(selected_id)) {
                edu.system.confirm("Bạn có chắc chắn muốn xóa dữ liệu hệ thống?");
                $("#btnYes").click(function (e) {
                    me.delete_HeSoLopHocPhan(selected_id);
                });
                return false;
            }

        });
        $('#tbldata_HSLHP').on('mouseenter', '.action-hslhp', function (event) {
            var strId = this.id;
            strId = edu.util.cutPrefixId(/remark/g, strId);
            strId = "#action" + strId;

            $(strId).removeClass("hide");
        }).on('mouseleave', '.action-hslhp', function (event) {
            var strId = this.id;
            strId = edu.util.cutPrefixId(/remark/g, strId);
            strId = "#action" + strId;
        ...
        $("#btnRewrite_HSLHP").click(function () {
            me.rewrite();
        });
        /*------------------------------------------
		--Discription: [2] action HocPhan
		-------------------------------------------*/
        $("#btnCallModal_HocPhan_HSLHP").click(function () {
            me.popup_HocPhan();
        });
        $("#tbldata_HocPhan_HSLHP").delegate(".btnSelect_HocPhan", "click", function () {
            var selected_id = this.id;
        ...
        $("#txtKeyword_HocPhan").on("keyup", function () {
            ...
            $("#tbldata_HocPhan_HSLHP tbody tr").filter(function () {
    ...
    page_load: function () {
        ...
        me.valid_TKKBP = [
            { "MA": "dropHocKy_HSLHP", "THONGTIN1": "1" },
            { "MA": "txtHeSo_HSLHP", "THONGTIN1": "1" },
            { "MA": "dropKieuHoc_HSLHP", "THONGTIN1": "1" },
            { "MA": "dropKhoanThu_HSLHP", "THONGTIN1": "1" },
    ...
    rewrite: function () {
        //reset id
        var me = this;
        var arrId = ["dropHocKy_HSLHP", "txtHeSo_HSLHP", "dropKhoanThu_HSLHP", "dropKieuHoc_HSLHP"];
        edu.util.resetValByArrId(arrId);
        //reset table
        me.arrHocPhan_Id = [];
        $("#tbldata_HocPhan_HSLHP td .btnClose").each(function () {
            $(this).trigger("click");
        });
    ...
    popup_HocPhan: function () {
        ...
        $("#myModalChuongTrinh_HSLHP").modal("show");
    },
    genPathSelect: function () {
        var me = this;
        //1.get text
        var strHe = edu.util.getTextById_Combo("dropHeDaoTao_HSLHP");
        var strKhoa = edu.util.getTextById_Combo("dropKhoaDaoTao_HSLHP");
        var strNganh = edu.util.getTextById_Combo("dropChuongTrinhDaoTao_HSLHP");
        if (!edu.util.checkValue(edu.util.getValById("dropHeDaoTao_HSLHP"))) {
            strHe = "..";
        }
        if (!edu.util.checkValue(edu.util.getValById("dropKhoaDaoTao_HSLHP"))) {
            strKhoa = "..";
        }
        if (!edu.util.checkValue(edu.util.getValById("dropChuongTrinhDaoTao_HSLHP"))) {
            strNganh = "..";
        }
    ...
    /*------------------------------------------
	--Discription: [1] ACCESS DB HeSoLopHocPhan
	--ULR:  
	-------------------------------------------*/
    save_HeSoLopHocPhan: function () {
        me = this;

        var strHocKy_Ids = edu.util.getValById("dropHocKy_HSLHP");
        var dHeSo = edu.util.convertStrToNum($("#txtHeSo_HSLHP").val());
        var strLoaiKhoan_Ids = edu.util.getValCombo("dropKhoanThu_HSLHP");
        var strKieHoc_Ids = edu.util.getValCombo("dropKieuHoc_HSLHP");
    ...
    update_HeSoLopHocPhan: function (arrId, dHeSo, strIds) {
        ...
        var obj_save = {
            ...
            'strPhamViApDung_Id': edu.util.getValById("dropChuongTrinhDaoTao_HSLHP"),
            'strPhanCapApDung_Id': "",
            'strNgayApDung': "",
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    me.affectUpdate_HeSoLopHocPhan(strIds, dHeSo);
                    //notify alert)timer
                    var obj = {
                        title: "Thông báo",
    ...
    delete_HeSoLopHocPhan: function (strId) {
        var me = this;
        var obj_save = {
    ...
    getList_HeSoLopHocPhan: function () {
        var me = this;
        var strTuKhoa = "";
        var pageIndex = 1;
        var pageSize = 10000;
        var strDaoTao_ToChucChuongTrinh_Id = edu.util.getValById("dropChuongTrinhDaoTao_HSLHP");
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    me.dtHeSoLopHocPhan = dtResult;
                    me.genTable_HeSoLopHocPhan();
                }
                else {
    ...
    /*------------------------------------------
	--Discription: [1] GEN HTML HeSoLopHocPhan
	--ULR:  
	-------------------------------------------*/
    genTable_HeSoLopHocPhan: function () {
        var me = this;
        //1. variable
        var $table_thead = "#tbldata_HSLHP thead";
        var $table_tbody = "#tbldata_HSLHP tbody";
        var thead = "";
        var tbody = "";
        $($table_thead).html("");
        $($table_tbody).html("");
        //2. thead
        thead = me.genThead_HeSoLopHocPhan(me.dtKhoanThu, me.dtKieuHoc, me.dtHeSoLopHocPhan);
        $($table_thead).append(thead);
        //3. tbody
        tbody = me.genTbody_HeSoLopHocPhan(me.dtHocPhan, me.dtHeSoLopHocPhan, me.dtKhoanThu, me.dtKieuHoc);
        //4. gen table
        $($table_tbody).append(tbody);
    },
    genThead_HeSoLopHocPhan: function (dtKhoanThu, dtKieuHoc, dtHeSoLopHocPhan) {
        var me = this;
        var arrLoaiKhoan = [];
        //thead
        var thead = '';
        ...
        arrLoaiKhoan = me.getUnique_LoaiKhoan(dtKhoanThu, dtHeSoLopHocPhan);
    ...
    genTbody_HeSoLopHocPhan: function (dtHocPhan, dtHeSoLopHocPhan, dtKhoanThu, dtKieuHoc) {
        var me = this;
        //tbody
        var tbody = '';
        var arrHocKy = [];
        var arrHeSoLopHocPhan = [];
        var arrHocPhan = [];
        var arrLoaiKhoan = [];
        var objHocKy = {};
        var rowspan = 0;
        ...
        function getUnique_Data() {
            for (var hp = 0; hp < dtHocPhan.length; hp++) {
                //I. processing 
                for (var dt = 0; dt < dtHeSoLopHocPhan.length; dt++) {
                    if (dtHocPhan[hp].DAOTAO_HOCPHAN_ID == dtHeSoLopHocPhan[dt].DAOTAO_HOCPHAN_ID) {
                        //1. get unique hocky
                        objHocKy = {};
                        objHocKy.ID = dtHeSoLopHocPhan[dt].DAOTAO_THOIGIANDAOTAO_ID;
                        objHocKy.TEN = dtHeSoLopHocPhan[dt].DAOTAO_THOIGIANDAOTAO_HOCKY;

                        if (!edu.util.objEqualVal(arrHocKy, "ID", objHocKy.ID)) {//if not exit
                            arrHocKy.push(objHocKy);
                        }
                        //2. get unique HeSoLopHocPhan
                        arrHeSoLopHocPhan.push(dtHeSoLopHocPhan[dt]);
                ...
                arrLoaiKhoan = me.getUnique_LoaiKhoan(dtKhoanThu, dtHeSoLopHocPhan);
                //II. call gen tbody
                genTbody(arrHocKy, arrHocPhan, arrHeSoLopHocPhan, arrLoaiKhoan);
                //II. reset
                arrHeSoLopHocPhan = [];
                arrHocKy = [];
                arrHocPhan = [];
        ...
        function genTbody(arrHocKy, arrHocPhan, arrHeSoLopHocPhan, arrLoaiKhoan) {
            ...
            for (var hk = 0; hk < arrHocKy.length; hk++) {//1.loop HocKy
                ...
                for (var lk = 0; lk < arrLoaiKhoan.length; lk++) {//2.loop LoaiKhoan
                    for (var kh = 0; kh < dtKieuHoc.length; kh++) {//3.loop KieuHoc
                        ...
                        for (var dt = arrHeSoLopHocPhan.length -1; dt >=0; dt--) {//4.loop HeSoLopHocPhan
                            if (arrHocKy[hk].ID == arrHeSoLopHocPhan[dt].DAOTAO_THOIGIANDAOTAO_ID
                                && arrHeSoLopHocPhan[dt].TAICHINH_CACKHOANTHU_ID == arrLoaiKhoan[lk].ID
                                && arrHeSoLopHocPhan[dt].KIEUHOC_ID == dtKieuHoc[kh].ID) {
                                if (arrKieuHoc.indexOf(dtKieuHoc[kh].ID) != -1) continue;
                                arrKieuHoc.push(dtKieuHoc[kh].ID);
                                //has value
                                dHeSo = edu.util.formatCurrency(edu.util.returnZero(arrHeSoLopHocPhan[dt].HESO));
                                //gen tr
                                tbody += genRow(arrId, dHeSo, arrHeSoLopHocPhan[dt].ID);
                                check = true;
                            }
                        }
                        if (!check) {//no value
        ...
        function genRow(arrId, dHeSo, strHeSo_Id) {
            var tbody = '';
            tbody += '<td class="td-center action-hslhp" id="remark' + arrId + '">';
            tbody += '<span id="zoneDisplay' + arrId + '">';
            tbody += '<span id="value' + arrId + '">' + dHeSo + "</span> <br />";
            tbody += '<span class="hide" id="action' + arrId + '">';
            tbody += '<a id="edit_' + arrId + '" class="btn btn-default btn-circle btnEdit_HSLHP" title="Chỉnh sửa"><i class="fa fa-pencil color-active"></i></a>';
            tbody += '<a id="delete_' + arrId + '" class="btn btn-default btn-circle btnDel_HSLHP" title="Xóa"><i class="fa fa-times-circle color-active"></i></a>';
            tbody += '</span>';
            tbody += '</span>';
            tbody += '<span id="zoneEdit' + arrId + '"  style="display: none">';
            tbody += '<input type="text" id="txt' + arrId + '" class="form-control btnUpdate_HSLHP" data-ax5formatter="money"/>';
            tbody += '</span>';
            tbody += '</td>';
            return tbody;
    ...
    getUnique_LoaiKhoan: function (dtKhoanThu, dtHeSoLopHocPhan) {
        var me = this;;
        var arrLoaiKhoan = [];

        for (var lk = 0; lk < dtKhoanThu.length; lk++) {

            for (var dv = 0; dv < dtHeSoLopHocPhan.length; dv++) {

                if (dtHeSoLopHocPhan[dv].TAICHINH_CACKHOANTHU_ID == dtKhoanThu[lk].ID) {
    ...
    /*------------------------------------------
	--Discription: [1] ACTION HTML - HeSoLopHocPhan
	--ULR:  
	-------------------------------------------*/
    openBoxEdit_HeSoLopHocPhan: function (arrId) {
    ...
    closeBoxEdit_HeSoLopHocPhan: function (arrId) {
        var $placeEdit = "#zoneEdit" + arrId;
        var $placeDisplay = "#zoneDisplay" + arrId;
        var $input = "#txt" + arrId;
        $($placeEdit).hide(120);
    ...
    affectUpdate_HeSoLopHocPhan: function (strIds, value) {
        var me = this;
        //1. add or update remark
        me.removeAffect_HeSoLopHocPhan(strIds);
    ...
    affectDelete_HeSoLopHocPhan: function (arrId) {
        //2. update html new value
        var $empty = "#value" + arrId;
        $($empty).html("");
    },
    removeAffect_HeSoLopHocPhan: function (arrId) {
    ...
    getList_HeDaoTao: function () {
        var me = main_doc.HeSoLopHocPhan;
        var obj_HeDT = {
    ...
    cbGenCombo_HeDaoTao: function (data) {
        ...
        var obj = {
            ...
            renderPlace: ["dropHeDaoTao_HSLHP"],
            type: "",
    ...
    cbGenCombo_KhoaDaoTao: function (data) {
        ...
        var obj = {
            ...
            renderPlace: ["dropKhoaDaoTao_HSLHP"],
            type: "",
    ...
    cbGenCombo_ChuongTrinhDaoTao: function (data) {
        ...
        var obj = {
            ...
            renderPlace: ["dropChuongTrinhDaoTao_HSLHP", "dropChuongTrinhDaoTao_Form_HSLHP"],
            type: "",
    ...
    --ULR:  
    -------------------------------------------*/
    getList_LopHocPhan: function (strChuongTrinh_Id) {
        var me = main_doc.HeSoLopHocPhan;
        var obj = {
            strCoSoDaoTao_Id: "",
            strKhoaDaoTao_Id:"",
        ...
        edu.system.getList_LopHocPhan(obj, "", "", me.cbGenTreejs_HocPhan);
    },
    getList_HocPhan_OnModal: function (strChuongTrinh_Id) {
        ...
        edu.system.getList_LopHocPhan(obj, "", "", me.cbGenTable_HocPhan);
    },
    /*------------------------------------------
    ...
    genComBo_KhoanThu: function (data) {
        ...
        var obj = {
            ...
            renderPlace: ["dropKhoanThu_HSLHP"],
            type: "",
    ...
    cbGenTreejs_HocPhan: function (data) {
        ...
        $('#zone_treejs_hocphan').on("select_node.jstree", function (e, data) {
            ...
            //2.3 if zone-input-hslhp is open --> close
            edu.util.toggle_overide("zone-hsph", "zone_list_hslhp");
    ...
    cbGenTable_HocPhan: function (data) {
        var me = this;
        var jsonForm = {
            strTable_Id: "tbldata_HocPhan_HSLHP",
            aaData: data,
            "sort": true,
            colPos: {
                left: [1],
