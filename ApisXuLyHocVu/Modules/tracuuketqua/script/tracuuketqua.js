...
TraCuuKetQua.prototype = {
    ...
    init: function () {
        ...
        me.strHead = $("#tblTraCuuKetQua thead").html();
        ...
        $("#tblTraCuuKetQua").delegate('.btnDetail', 'click', function (e) {
            $('#myModal').modal('show');
            me.getList_QuanSoTheoLop(this);
        });
        $("#tblTraCuuKetQua").delegate('.btnChangeMucXuLy', 'click', function (e) {
            $('#myModal').modal('show');
            var obj = edu.util.objGetOneDataInData(this.id, me.strTraCuuKetQua, "ID");
            me.strTraCuuKetQua = obj.ID;
        ...
        $("#tblTraCuuKetQua").delegate('#chkSelectAll_TraCuuKetQua', 'click', function (e) {
            edu.util.checkedAll_BgRow(this, { table_id: "tblTraCuuKetQua" });
        });
        $("[id$=chkSelectAll_TraCuuKetQua]").on("click", function () {
            edu.util.checkedAll_BgRow(this, { table_id: "tblTraCuuKetQua" });
        });
        ...
        edu.system.getList_MauImport("zonebtnTC", function (addKeyValue) {
    ...
    genList_TrangThaiSV: function (data) {
        var me = this;
        main_doc.TraCuuKetQua.dtTrangThai = data;
        var row = '';
    ...
    getList_TraCuuKetQua: function (strDanhSach_Id) {
        var me = this;
        //--Edit
        var obj_list = {
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    me.dtTraCuuKetQua = dtReRult;
                    me.genTable_TraCuuKetQua(dtReRult);
                }
                else {
    ...
    getData_TraCuuKetQua: function (jsonSV, strTuKhoaXuLy) {
        var me = this;
        console.log(jsonSV)
        //--Edit
        var obj_list = {
    ...
    getList_KeHoachXuLy: function (strDanhSach_Id) {
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    var dtReRult = data.Data;
                    me.genCombo_KeHoachXuLy(dtReRult, data.Pager);
                }
                else {
    ...
    --ULR: Modules
    -------------------------------------------*/
    genTable_TraCuuKetQua: function (data, iPager) {
        var me = this;
        $("#tblTraCuuKetQua thead").html(me.strHead);
        var row = '';
        ...
        $("#tblTraCuuKetQua thead tr:eq(1)").append(row);
        $("#tblTraCuuKetQua thead tr:eq(0)").append('<th rowspan="2" class="td-center td-fixed" > <input type="checkbox" id="chkSelectAll_TraCuuKetQua"></th>');

        var jsonForm = {
            strTable_Id: "tblTraCuuKetQua",

            bPaginate: {
                strFuntionName: "main_doc.TraCuuKetQua.getList_TraCuuKetQua()",
                iDataRow: iPager,
            },
        ...
        for (var i = 0; i < data.rsCotThongSoXuLy.length; i++) {
            jsonForm.aoColumns.push({
                "mRender": function (nRow, aData) {
                    return '<span id="lbl' + aData.ID + '_' + main_doc.TraCuuKetQua.dtTraCuuKetQua.rsCotThongSoXuLy[edu.system.icolumn++].TUKHOA + '"> </span>';
                }
            });
        ...
        for (var i = 0; i < data.rsThongTinNguoiHoc.length; i++) {
            for (var j = 0; j < data.rsCotThongSoXuLy.length; j++) {
                me.getData_TraCuuKetQua(data.rsThongTinNguoiHoc[i], data.rsCotThongSoXuLy[j].TUKHOA);
            }
        }
    },
    /*------------------------------------------
