...
HoSoDanhSach.prototype = {
    ...
    save_HS: function () {
        ...
        var obj_save = {
            ...
            'strDiaChiCoQuanCongTac': edu.util.getValById('txtDiaCoQuan'),
            'strMaSoThueCaNhan': edu.util.getValById('txtMaSoThue'),
            'strMaSoThueCoQuan': edu.util.getValById('txtMaSoThueCoQuan'),
    ...
    viewForm_HS: function (data) {
        ...
        edu.util.viewValById("txtDiaCoQuan", data.DIACHICOQUANCONGTAC);
        edu.util.viewValById("txtMaSoThue", data.MASOTHUECANHAN);
        edu.util.viewValById("txtMaSoThueCoQuan", data.MASOTHUECOQUAN);
    },
    save_ThanhVien: function (strThanhVien_Id, strSinhVien_Id) {
        var me = this;
        var strId = strThanhVien_Id;
        var strTPGD_HoDem = edu.util.getValById('txtHoDem' + strThanhVien_Id);
    ...
    genHTML_ThanhVien: function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            row += '<td><input type="text" id="txtHoDem' + strThanhVien_Id + '" value="' + edu.util.returnEmpty(data[i].HODEM) + '" class="form-control"/></td>';
    ...
    genHTML_ThanhVienNew: function (strThanhVien_Id) {
        ...
        row += '<td><select id="dropQuanHe' + strThanhVien_Id + '" class="select-opt"></select ></td>';
        row += '<td><input type="text" id="txtHoDem' + strThanhVien_Id + '" class="form-control"/></td>';
        ...
        edu.system.makeRequest({
            success: function (data) {
                if (data.Success) {
                    ...
                    me.genTable_QuyetDinh(dtResult);
                }
                else {
                    edu.system.alert(obj_list.action + ": " + data.Message, "w");
                }
    ...
    reportDanhMuc: function (aData, strEmail, strMaDanhMuc) {
        ...
        edu.system.loadToCombo_DanhMucDuLieu(strMaDanhMuc, "", "", dtDanhMuc => {
            edu.system.report("DANHMUC_DON", "", function (addKeyValue) {
                ...
                addKeyValue("strFileName", edu.util.returnEmpty(dtDanhMuc[0]["THONGTIN5"]));
                addKeyValue("strEmail", strEmail);
                addKeyValue("strTieuDe", edu.util.returnEmpty(dtDanhMuc[0]["THONGTIN3"]));
                addKeyValue("strNoiDung", edu.util.returnEmpty(dtDanhMuc[0]["THONGTIN4"]));
                dtDanhMuc.forEach(e => {
                    addKeyValue(e.MA, eval(e.TEN));
                });
            });
        }, "");
    }
